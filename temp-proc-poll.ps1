...
for ($i = 0; $i -lt 40; $i++) {
  ...
  Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.ProcessName -match 'Geuldobi|electron|crashpad|backend' } | ForEach-Object {
    $key = "$($_.ProcessName)-$($_.Id)"
    if (-not $matches.ContainsKey($key)) {
      $matches[$key] = [pscustomobject]@{SeenAt=(Get-Date); ProcessName=$_.ProcessName; Id=$_.Id; Path=$_.Path }
    }
  }
}
