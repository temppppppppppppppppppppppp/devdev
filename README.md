...
- 회차 단위 전면 재생성
- 앙상블 후보군 전면 재생성
- SQLite 프로젝트 DB로 실행 상태와 산출물 메타데이터 저장
- sqlite-vec 기반 벡터 메모리로 과거 회차 맥락 검색
- 세션 메모리로 실행 중 맥락 유지
- 컨텍스트 캐싱으로 반복 맥락 재사용
...
| S4 | 최적화 중기 진입. 세션 메모리, sqlite-vec 벡터 메모리, 컨텍스트 캐싱을 통해 재시도율 2회 수렴을 노릴 수 있는 단계다. |
...
| `projects/` | 실제 파이프라인 실행 결과, SQLite DB, 로그, 원고 산출물이 쌓이는 영역이다. |
| `modules/` | S2/S3/S4 런타임, 에이전트, 검증기, DB, sqlite-vec 벡터 메모리, 캐시, 세션 메모리 등 핵심 코드가 있는 영역이다. |
