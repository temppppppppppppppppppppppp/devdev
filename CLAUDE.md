...
- Stage4 advisory 체인 — TruthGate(LM-A) → NpcDriftAdvisor(LM-B, 4필드 확장) → NumericDriftAdvisor(LM-C) → FlashbackVerifier(LM-E, 원문 대조 LM-H) → InfoParadoxChecker(LM-F, 1인칭 전용) → RelationshipDriftAdvisor(LM-D). 전부 `_director_mc_parts`에 주입, Director 최종 판정.
