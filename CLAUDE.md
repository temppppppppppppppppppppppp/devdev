...
1. **코드 수정 전 Plan Mode 필수** — 코드를 변경하는 모든 작업은 반드시 Plan Mode(`/plan`)로 진입하여 계획을 수립하고 사용자 승인 후 실행. 단순 오타 수정, 1줄 변경 등 자명한 작업은 예외.
2. **감사/조사 작업은 TF 프레임워크** — 전수조사, 감리, 감사 작업은 TF(Task Force) 문서 기반으로 수행. 트랙 분할 → 병렬 조사 → 교차검증 → 오탐제거 3pass 절차 준수.
3. **산출물은 docs/{날짜}/ 하위** — 모든 조사·감사 산출물은 `docs/YYYY-MM-DD/` 디렉토리에 마크다운으로 작성.
