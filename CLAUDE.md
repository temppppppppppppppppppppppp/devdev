...
- NC-1 응답 의무 — Director가 NC 항목 각각에 AGREE/DISMISS + 이유를 `numeric_consistency_review` 필드에 필수 응답. AGREE 시 contradiction_check에 반영 + continuity_contradiction 점수 상한 제한(1건당 -8점). 미응답 시 python_warnings 감점(10→5). Director 주권 존중하되 묵살 금지.
- NC-3 일관성 체크리스트 — Director JSON 응답에 `consistency_checklist` 필드 추가. 10개 카테고리(numeric_accuracy/arithmetic/title_consistency/scene_overlap/percent_calculation/event_ordering/space_continuity/npc_identity/time_progression/opening_diversity)를 매 심사마다 OK/ISSUE로 체크. ISSUE 3건+ → python_warnings 상한 3점. 체크리스트 누락 → 감점 없음(안정화 기간). `DIRECTOR_AUDIT_SCHEMA` optional, 반환 dict에 전파.
