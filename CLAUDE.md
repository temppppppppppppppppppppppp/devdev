...
- **현재 단계**: TF-45/47/48 완료. 비무협 무협 오염 근절 + InPlace 원고 보호 + Arc 실행 상태 연속성. **TF-54(WritingDirective) 명세 문서화 완료, 구현 대기.**
- **테스트 기준선**: **3,170 passed + 0 xfailed** (last verified 2026-03-03, `pytest tests/ -q`)
...
- **TF-54 구현 명세**: `docs/2026-03-04/TF-54-writing-directive-spec.md` (WritingDirective — 패턴 추적 + 에피소드별 동적 집필 지시, 5단계 54a~54e)
...
- **LLM 모델 설정 SSOT 미완 (기술 부채)** — 모델명이 `config/models.yaml` + `modules/core/constants.py` + `modules/core/config_manager.py` + 개별 `.py` default 인자 등 **다수 파일에 분산 하드코딩**되어 있다. 모델 교체 시 `models.yaml`만 바꾸면 안 되고, `constants.py`(ModelTiers 11곳) + `config_manager.py`(5곳) + `blueprint_ensemble.py`/`three_phase_blueprint_generator.py`/`state_locked_arc_generator.py`/`narrative_structure_analyzer.py`/`self_reflection.py`/`tree_of_thoughts.py` default 인자까지 전부 교체해야 함. `base_agent.py:47` fallback_chain 딕셔너리 키와 `metrics_collector.py` 가격표는 변경 불필요. **향후 SSOT화 필요** — `models.yaml` 단일 참조로 통일.
...
- Stage4 advisory 체인 — TruthGate(LM-A) → NpcDriftAdvisor(LM-B, 4필드 확장) → NumericDriftAdvisor(LM-C, MAX_ITEMS=30/MAX_HISTORY_POINTS=20) → FlashbackVerifier(LM-E, 원문 대조 LM-H) → InfoParadoxChecker(LM-F, 1인칭 전용) → RelationshipDriftAdvisor(LM-D) → LongTermRepetitionAdvisor(P1-5, 20화+ 전용). 전부 `_director_mc_parts`에 주입, Director 최종 판정.
...
| 100 | TF-54 | WritingDirective — 패턴 추적(54a) + 지시 생성(54b) + CW 주입(54c) + Director 정합성(54d) + self-critique(54e) | 📋 명세 완료, 구현 대기 (`docs/2026-03-04/TF-54-writing-directive-spec.md`) |
