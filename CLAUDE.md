...
- **완료된 것**: Phase 1~2, 5-A/5-B/5-C, 6-A/6-B/6-C, 4C(DI), 4D(sqlite-vec), 3-5B(패치), 3-5A(NPC이력), **4-R1~R3(몬스터 분할)**, R4-a(NO-GO), **3-QR(품질 회귀 감지)**, **3-5C(NPC 과잉 경고)**
- **약점**: 플롯 중복 감지 불안정 (Chain 1)
- **현재 단계**: Phase 3-5C 완료 → 관측성 개선 또는 B. 크로스 반복 감지
- **다음 우선순위**: 관측성 개선 (perf_timer) 또는 B. 크로스 에피소드 반복 감지
- **테스트 기준선**: unit 38 + quality 15 + pipeline 89 + E2E 22 + regression 94 = **258 passed**
