...
class BibleExtractor(BaseAgent):
    ...
    def __init__(self, context, client, model_tier: str = "gemini-2.5-pro"):
        ...
        # 장르별 HUD 타입
        self.genre_hud_map = {"wuxia": "MartialHUD", "hunter": "HunterHUD", "investment": "FinanceHUD"}

        # 추출 통계
        self.stats = {"chunks_processed": 0, "npcs_found": 0, "items_found": 0, "seeds_found": 0}
...
"""
        ...
        if isinstance(result, dict):
            print("      [OK] 주인공 HUD 추출 완료")
            return result

        print("      [WARN] 주인공 HUD 추출 실패")
    ...
    def _assemble_bible(
        ...
        for npc in npcs:
            key_npc = {
                ...
                "role": npc.get("role", ""),
                "desc": npc.get("desc", ""),
                f"NPC_{genre.capitalize()}_HUD": {"status": npc.get("status", "활동중"), "trait": npc.get("trait", "")},
            }
        ...
        bible = {
            "MasterBible": {
                ...
                hud_type: {"Protagonist": protagonist_hud},
                "WorldState": {
