...
"""
    ...
    def phase2_generate_details(self, skeleton: list) -> list:
        ...
        for batch_idx in range(total_batches):
            ...
            # 현재 배치 생성
            batch_details = self._generate_batch_details(
                batch_skeleton,
                prev_summary,
                start + 1
            )
...
"""
    ...
    def _summarize_previous_blocks(self, blocks: list) -> str:
        ...
        for b in blocks:
            ...
            summaries.append(
                f"- {b['block_id']}: {content.get('reward', '')[:100]}"
            )
        return "\n".join(summaries)
...
"""
    ...
    def save_final(self, blocks: list):
        ...
        output = {
            ...
            "work_name": self.bible.get('MasterBible', {}).get('ProjectData', {}).get('MetaInfo', {}).get('title', '미정'),
            "genre": self.genre,
