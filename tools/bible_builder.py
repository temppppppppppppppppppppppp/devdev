...
class BibleBuilder:
    ...
    def step1_basic_info(self) -> dict:
        ...
        # 아키타입
        archetype = self._show_options(
            self.genre_info["archetypes"],
            f"🎭 장르 아키타입 ({self.genre_info['name']})"
        )
...
"""
    ...
    def step5_world_state(self, hud: dict) -> dict:
        ...
        # 기본 설정
        setting = self._show_options(
            self.genre_info["typical_settings"],
            "🏛️ 배경 선택"
        )
...
"""
    ...
    def run(self):
        ...
        # Step 3: 상업적 코드
        components["commercial_code"] = self.step3_commercial_code(
            components["meta_info"],
        ...
        # Step 6: NPC
        components["npcs"] = self.step6_npcs(
            components["meta_info"],
            components["core_identity"]
        )

        # Step 7: 역사 이벤트 (투자물)
        components["historical_events"] = self.step7_historical_events(
        ...
        # Step 9: 복선 씨앗
        components["seeds"] = self.step9_seeds(
            components["npcs"],
            components["meta_info"]
        )
...
def main():
    ...
    parser.add_argument("--genre", default="investment",
                        choices=["investment", "wuxia", "hunter"],
                        help="장르 (기본: investment)")
