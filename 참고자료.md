...
> **문서 성격**: 시스템 아키텍처, 코드 조사 결과, 리팩토링 로드맵, 개선 아이디어를 통합 관리하는 **종합 참고자료**.
> **기준**: V65+ 코드 (2026-02 기준). 리팩토링 최신 스냅샷은 아래 "2026-02-16 업데이트" 기준.
> **대상 독자**: 후임 에이전트. 이 문서를 먼저 읽고 작업에 착수할 것.
> **최종 갱신**: 2026-02-16

### ✅ 2026-02-16 업데이트 (최신 스냅샷)

- **Phase 4C 완료**: Stage2/3/4 Orchestrator DI 전환 완료
  - 커밋: `3667d11` → `6a1bf98` → `6a23e41` → `fa621fe` → `2c0193e`
  - 보강: `493e104` (targeted ruff/pre-commit unblock)
- **Phase 4D 완료**: ChromaDB → sqlite-vec 마이그레이션 + 레거시 완전 제거
  - `memory_engine.py`, `blueprint_memory.py` 삭제
  - VecMemory(`vec_memory.py`) 단일 벡터 경로 확정
  - ChromaDB 실행 참조 0건
- **God Object 핵심 지표 개선**: `self.app` 직접 참조 651건 → 0건 (stage2/3/4 합산)
- **Phase 3-5A 완료** (2026-02-14): NPC append-only 이력 DB + 검증 파이프라인 통합
  - 3-5A-1: `npc_history` 테이블 + StateTracker `bind_db()` + 기록 API (`126dceb`)
  - 3-5A-2: blocking_validator 사망NPC 회상/행동 구분, continuity_validator 성격급변 감지, stage4 npc_history 주입 (`4d669a9`)
- **Phase 5-B 완료** (2026-02-14): Settings/가드 외부화
  - 5-B-1: `config/settings/validation.yaml` 생성 + ConfigManager 확장 (load_settings, get_guard_threshold, get_feature_flag) (`e6f0f31`, `7b8a148`)
  - 5-B-2: 4개 validator 하드코드 임계값 → `_threshold()` 헬퍼로 ConfigManager 경유 치환 (`352efcc`, `afc1765`, `f678c9a`, `a389ac5`)
  - `modules/validation/threshold_helper.py` 공유 모듈 추출
- **현재 단계**: 백로그 전량 소진 → 선택 작업(R5 계속 / Guard YAML / L3 확장) 대기
- **완료**: 6-B(E2E 24), 4-R1~R3(몬스터 분할), R4-a(NO-GO), D-1~D-4, E-1, B-2
- **테스트 기준선**: `pytest tests/ -q` → **1,593+ passed, 68 xfailed** (checkpoint `1818357`)
- **E-1 완료**: `380c911` — Silent Pass 16건 로깅 보강 완료
- **D-1 완료**: `f0390e8` — POV 시스템 활성화 완료 (YAML 연동 불필요)
- **D-2 완료**: `9ba06cd` — 에피소드 롤백 NPC 되감기 완료
- **D-3 완료**: `79afbfd` — 문체 분석 → StyleGuard 자동생성 연동 완료
- **D-4 완료**: `a2c1fb2` — Director 선택 추적/승률 조회 완료
- **B-2 완료**: `d45d53c` — Protocol 미적합 5개 클래스 어댑터 완료
- **실행 우선순위 SSOT**: `내일작업.md`를 기준으로 작업 순서 결정
- 주의: 아래 일부 표/다이어그램은 과거 시점 수치를 포함하며, 최신 상태는 본 섹션을 우선한다.

### ⚖️ 시스템 대원칙

1. **Python은 수집만, 판단은 LLM이** — Python 코드는 데이터 수집·포맷팅·전달만 수행. "이게 오류인가?", "수정해야 하는가?" 같은 **판단은 반드시 LLM 에이전트**가 담당.
2. **팩트시트 수정 권한은 LLM만** — NPC 속성, 세계관 설정, 관계도 등 팩트시트를 수정할 수 있는 건 LLM 에이전트뿐. Python 로직이 자동으로 팩트를 덮어쓰면 안 됨.
3. **디렉터 주권주의 (내각제)** — Director가 최종 품질 결정권을 가짐. Chief Writer, Analyst 등은 초안 제출만 하고, **합격/불합격/수정 지시는 Director가 내린다.** 다른 에이전트가 Director를 우회하면 안 됨.
4. **사망 캐릭터는 회상/언급만 허용** — `deceased=True`인 NPC가 행동/대사로 등장하면 REJECT. 회상·과거 장면·타인의 언급은 허용.
...
graph LR
    ...
    S2["stage2_orch 341건"] -->|"self.app"| APP
    S4["stage4_orch 312건"] -->|"self.app"| APP
```

> ✅ Phase 4C에서 `self.app` → 의존성 주입(DI) 전환 완료 (stage2/3/4 핵심 경로)
...
[S] = Sonnet 4.5로 충분    [O] = Opus 4.6 + TF(계획서) 필요

| 순서 | 항목 | 상태 | 난이도 |
|------|------|------|--------|
| 1 | Phase 1 / 1.5 (에러 핸들링 + 레거시 정리) | ✅ 완료 | ★☆☆☆☆ |
| 2 | Phase 2-A / 2-B / 2-C (Pydantic + TypeHints + ABC/Protocol) | ✅ 완료 | ★★★☆☆ |
| 3 | Phase 2.5 / 4D (sqlite-vec SSOT + 벡터 경로 재연결) | ✅ 완료 | ★★★★☆ |
| 4 | Phase 3-5B (수정 모드: Stage4 패치 모드) | ✅ 완료 | ★★★☆☆ |
| 5 | Phase 3-5A (NPC 이력 + 검증 강화) | ✅ 완료 | ★★★★☆ |
| 6 | Phase 5-B (Settings/가드 외부화) | ✅ 완료 | ★★★☆☆ |
| 7 | Phase 6-B (E2E 테스트) | ✅ 완료 (24 E2E, `1678550`+`1818357`) | ★★★★☆ |
| 8 | Phase 4 잔여 (R1~R3 분할 완료, R4 NO-GO) | ✅ 완료 | ★★★★★ |

> [!NOTE]
> 기존 상세 로드맵(Phase 1~6)은 역사 기록으로 유지한다. 실행 우선순위 판단은 본 표와 하단 `미완료 Phase` 표를 기준으로 한다.
...
> | 2-A (Pydantic) | `RISKY` | Phase 1 완료 ✅ | ✅ 완료 | `domain/models.py`, 에이전트 전체 |
> | 2-C (ABC/Protocol) | `RISKY` | 2-A 완료 ✅ | ✅ 완료 | `base_agent.py`, 에이전트 전체 |
> | 2.5/4D (DB 통합) | `RISKY` | Phase 2 완료 ✅ | ✅ 완료 | `vec_memory.py`, `project_data.db`, `memory/vec_memory.db` |
> | 3-5B (수정 모드) | `RISKY` | 4D 완료 ✅ | ✅ 완료 | `stage4_orchestrator.py`, `chief_writer.py`, `chief_writer.yaml` |
> | 3-5A (NPC 이력/검증) | `RISKY` | 3-5B ✅ | ✅ 완료 | `state_tracker*`, `db_manager.py`, `blocking_validator.py`, `continuity_validator.py` |
> | 4 (구조 개선 잔여) | `RISKY` | 6-B | 대기 | `main_a.py`, `stage2_orchestrator.py`, `stage4_orchestrator.py` |
> | 5-A (프롬프트 외부화) | `SAFE` | 없음 | ✅ 완료 | `config/prompts/*.yaml` (43개), `prompt_loader.py` |
> | 5-B (설정 관리) | `SAFE` | 3-5A ✅ | ✅ 완료 | `config_manager.py`, `validation.yaml`, `threshold_helper.py` |
> | 5-C (의존성 정리) | `SAFE` | 없음 | ✅ 완료 | `requirements.txt`, `.gitignore`, `pyproject.toml` |
> | 6-A (단위 테스트) | `SAFE` | 없음 | ✅ 완료 | `tests/` |
> | 6-B (E2E 테스트) | `RISKY` | 3-5A | ✅ 완료 (24 E2E) | 파이프라인 전체 |
> | 6-C (코드 품질) | `SAFE` | 없음 | ✅ 완료 | `pyproject.toml`, `.pre-commit-config.yaml` |
>
> **안전 작업 우선 순서** (현재 기준):
> ~~`5-A`~~ → ~~`5-A'`~~ → ~~`5-C`~~ → ~~`6-A`~~ → ~~`6-C`~~ → ~~`5-B`~~ ← **모두 완료**
...
### [역사 기록] Phase 2: 타입 시스템 도입 (당시 기준: 2-B ✅ / 2-A, 2-C 미착수)
...
### Phase 2.5: sqlite-vec 도입 (ChromaDB → SQLite 통합) — ✅ Phase 4D로 완료

> [!NOTE] **Phase 4D로 완료됨.** `memory_engine.py`/`blueprint_memory.py` 삭제, VecMemory 단일 경로. 아래는 초기 계획 기록.
...
| # | 시나리오 | 예시 | 현재 탐지 |
|---|---------|------|----------|
| 20 | 관계 유형 변조 | 스승-제자(3화) → 갑자기 고객-상인(15화) | ❌ 관계 이력 없음 |
| 21 | 호칭 불일치 | "사부님"(5화) → "아저씨"(10화) | ❌ |
| 22 | 적대→우호 급변 | 원수(8화) → 12화에 이유 없이 절친 | ⚠️ `emotion_tracker` 약함 |
| 23 | 일방적 관계 소멸 | 연인(10화) → 20화부터 서로 모르는 사이처럼 | ❌ |
| 24 | 관계 인물 혼동 | A의 스승이 B(3화) → 같은 스승이 C(15화) | ❌ |

> **원인**: `state_tracker_npc`가 NPC 간 **관계(edge)**를 이력으로 추적하지 않음. 최신 스냅샷만 존재.
...
2. NPC 관계 그래프 + 이력:
   - 관계 테이블: (npc_a, npc_b, relation_type, since_episode)
   - "스승-제자"(ep3) → "고객-상인"(ep15) 변경 감지
   - 관계 변경 시 → 스토리상 결별/전환 장면이 있는지 LLM 확인

3. continuity_inspector 강화:
...
> 가장 시급한 것: **① 고정 속성 변조** + **⑥ 관계도 변조** — NPC 이력 + 관계 테이블 추가로 커버.

**3-D. 수정 모드 (Revision Mode) — 전체 재작성 → 부분 수정 전환**

> [!NOTE] **최신은 `docs/phase3_5b_blueprint.md` 우선.** 아래는 초기 구상이며, 실제 구현 범위/정책은 5B 청사진에서 확정.
...
**3-E. 추가 개선 아이디어 (미래 Phase)**

**멀티 드래프트 비교 (Stage 4 확장)**

현재: Chief Writer가 1개 작성 → 합/불합 → 재작성. 개선: 2~3개 동시 생성 → Director가 최고점 선택 혹은 좋은 부분 조합. 이미 `arc_ensemble.py`, `blueprint_ensemble.py`에 앙상블 패턴이 있으므로 원고에도 확장 가능.

**컨텍스트 압축 고도화 (장편 대응)**

에피소드 50화+ 쌓이면 이전 원고 전부를 Context에 넣을 수 없음. `context_compression.py`가 있지만 — 에피소드별 **중요도 가중치**를 부여하여 "30화 전 사건이지만 현재 Arc에 직결되는 복선"은 원문 유지, "일상 에피소드"는 1줄 요약으로 압축.

**에피소드 의존성 그래프**

"15화에서 심은 복선이 23화, 45화에서 회수됨" → 그래프로 추적하면 특정 에피소드 수정 시 **영향 범위 자동 파악** 가능. `foreshadow_tracker` 확장으로 구현 가능.

**품질 회귀 감지**

에피소드 N이 85점인데 N+1이 갑자기 60점 → 자동 플래그. "이전 에피소드 대비 품질 25점 하락. 계속하시겠습니까?" `pass_rate_monitor`에 에피소드 간 비교 로직 추가.

**LangGraph 도입 검토 → ❌ 미도입 결정 (2026-02-12)**

검토 결과 현재 시스템에 불필요:
- 이미 `stage2_orchestrator`, `stage4_orchestrator`가 커스텀 그래프 역할 수행
- 도입 시 2000줄+ 오케스트레이터 2개 전면 재작성 필요 → 비용 대비 이득 없음
- LangChain 생태계 의존성 추가 + Gemini 호환 불안정
- 필요한 건 "NPC 관계 그래프"(SQLite 테이블) + "에피소드 의존성 그래프"(`foreshadow_tracker` 확장)이며, 프레임워크 교체가 아님

**인간 편집 학습 (Human-in-the-Loop Feedback)**

사용자가 생성된 원고를 직접 수정하면 → diff 캡처 → "시스템이 X라고 썼는데 사용자가 Y로 바꿈" 패턴 학습 → 다음 프롬프트에 반영. `failure_learning.py`에 "사용자 선호 학습" 채널 추가. 예: "이 작가는 감정 묘사를 2줄 이상 원한다", "대사 비중을 30% 이상 유지한다" 등 자동 추출.

**드라이런 모드 (Dry Run)**

원고를 생성하되 **저장 안 함**. 품질 점수 + 잠재 문제(연속성 위반, 금기어 등)만 미리보기. Arc 단계에서 "이 Arc로 가면 예상 점수 70점" 알림 가능. 토큰은 쓰지만 실패 원고의 DB 오염 방지. 구현: `stage4_orchestrator`에 `dry_run=True` 플래그 → save 단계만 스킵.

**LLM 온도 자동 조절 (Adaptive Temperature)**

| 통과율 | 온도 조절 |
|-------|----------|
| < 30% | temperature 0.3 (보수적 — 일단 합격시키기) |
| 30~70% | temperature 0.7 (기본) |
| > 70% | temperature 0.9 (창의적 — 여유 있으니 실험) |

`pass_rate_monitor` 데이터 → `base_agent`의 LLM 호출 파라미터에 피드백. 현재는 temperature 하드코딩.

**수정 diff 뷰**

수정 모드(3-D)로 부분 수정 후 → 원본 vs 수정본 diff를 PD에게 표시. LLM이 "지시받지 않은 곳"까지 몰래 바꾸는 걸 감지. `difflib.unified_diff()`로 구현 가능. UI에 빨간색/초록색 하이라이트.

**토큰 예산 추적 (Token Budget Tracker)**

에피소드별 토큰 사용량 + 재시도 횟수 DB 기록 → "5화는 재시도 3번 해서 토큰 3배 소모" → 어떤 유형(전투? 감정? 대화?)이 비용을 먹는지 분석. `quality_dashboard`에 비용 메트릭 추가. 월간/프로젝트 단위 비용 리포트 가능.

**프롬프트 A/B 테스트 고도화**

`ab_testing.py`가 이미 존재하지만 활용도 낮음. 확장안:
- 프롬프트 변형(예: 지시문 순서 변경, 제약 조건 강조도 변경) 자동 생성
- 동일 Arc에 대해 A/B 원고 생성 → 점수 비교 → 승리 프롬프트 자동 채택
- 장기 데이터 축적 → "어떤 프롬프트 패턴이 무협에서 가장 잘 먹히는가" 분석

**자기진단 대시보드 (System Health Dashboard)**

현재 `quality_dashboard.py`는 원고 품질만 추적. 확장:
- **에이전트 상태**: 각 에이전트별 성공률/실패율/평균 응답 시간
- **체인 상태**: Chain 1~3(Tier 5) 활성/비활성 여부 실시간 모니터링
- **컨텍스트 건전성**: 프롬프트에 실제로 들어간 컨텍스트 항목 수 (5/8개 중 3개 실패 등)
- UI: `studio_dashboard.py` (Streamlit) 확장 또는 HTML 리포트

**에피소드 선택적 롤백**

현재 `auto_backtrack`이 있지만 기능이 약함. 개선:
- 특정 에피소드만 골라서 롤백 + 재생성
- 롤백 시 해당 에피소드 이후의 state_tracker / NPC 상태도 자동 되감기
- 복선 의존성 그래프 연동 → "15화 롤백하면 23화, 45화에 영향" 경고

**장르 가드 자동 학습**

기존 원고 코퍼스(사용자가 Step 0에서 업로드한 기존 소설)에서:
- 사용 어휘 빈도 → 자동 "허용어/비허용어" 초안 생성
- 문체 패턴 → "이 작가는 3인칭 전지적, 문장 평균 25자" 자동 추출
- `bible_extractor.py` 확장 또는 별도 `style_analyzer` 모듈

**장편 요약 에이전트 (Arc Summarizer)**

50화+ 장편에서 새 Arc 설계 시 이전 모든 Arc를 읽을 수 없음. 전용 요약 에이전트가:
- 완료된 Arc를 1~2문단으로 압축 (NPC 변화점 + 핵심 사건 + 복선 위치)
- 이 요약을 DB에 별도 저장 → 새 Arc 설계 시 경량 컨텍스트로 활용
- 현재 `context_compression.py`보다 더 **의도적이고 구조화된** 요약

**Slack/Discord 알림 고도화**

`slack_bot.py`가 이미 존재하지만 단순 알림 수준. 확장:
- 에피소드 완성 시 → 점수 + 주요 변경사항 요약 + diff 링크
- 품질 회귀(3-E) 감지 시 → 즉시 알림 + "롤백하시겠습니까?" 버튼
- 일간/주간 리포트: 생산량, 평균 점수, 토큰 사용량

**병렬 검증 파이프라인 최적화**

현재 `validation_orchestrator.py`가 **병렬 실패 → 순차 폴백** 패턴 사용. 개선:
- 검증 항목 간 **의존성 그래프** 정의 → 독립 항목만 병렬 실행
- 치명적 검증(연속성) 먼저, 부가 검증(스타일)은 치명적 통과 후 실행
- 조기 종료: 치명적 검증 불합격 시 나머지 검증 스킵 → 토큰 절약

**NPC 과잉 등장 경고**

한 에피소드에 신규 NPC 5명+ 동시 등장 → "독자 인지 부하 초과" 플래그. `state_tracker_npc`의 등장 데이터로 바로 구현 가능. 기준: 신규 NPC 3명/화 초과 시 경고.

**크로스 에피소드 반복 감지**

현재 `repetition_guard.py`는 **1화 내** n-gram만 체크. "같은 비유를 50화 동안 10번 씀" → 에피소드 **간** 반복은 못 잡음. DB에 에피소드별 주요 표현/비유 인덱싱 → 전체 작품 레벨 반복 감지.

**클리프행어 패턴 추적**

| 패턴 | 경고 |
|------|------|
| 5화 연속 절체절명 엔딩 | "독자 피로 — 이완 에피소드 필요" |
| 10화째 긴장감 없는 엔딩 | "훅이 없음 — 클리프행어 삽입 권장" |
| 같은 유형 반복 (매번 "절벽 추락") | "클리프행어 다양성 부족" |

에피소드 마지막 문단 감정 분석. `pacing_analyzer.py` 확장.

**캐릭터 스크린타임 밸런스**

주인공이 3화 연속 등장 안 하거나, 조연이 갑자기 주인공보다 분량 압도 → 경고. NPC별 에피소드당 단어 수 추적. `state_tracker_npc`에 `word_count_per_episode` 필드 추가.

**대사 화자 명확성 체크**

긴 대화 장면에서 화자 지칭 없이 대사 3줄+ 연속 → "누가 말하는 건지 모호" 플래그. `chief_writer` 검증 단계에 정규식 기반 체크 추가.

**Gemini Context Caching 활용 ✅ 구현 완료**

Gemini API Context Caching 기능 → **토큰 비용 절감**. `base_agent.py` L920~1068에 `_get_or_create_context_cache` + `_ask_with_cached_context` 구현 완료. `chief_writer.py` (L223, L387) 및 `director_continuity.py` (L591, L701, L733)에서 실사용 중.

**댓글/여론 반응 생성 (현대 연예계물 전용)**

현대 연예계 장르에서 방송·언론 노출 장면 시 → 작중 인터넷 댓글, SNS 반응, 기사 댓글을 자동 생성:

```
장면: 주인공이 음악방송 1위

→ 자동 생성:
ㄴ ㅇㅇ(112.34) 와 진짜 소름... 이 무대 레전드다
ㄴ ㅇㅇ(223.45) 아직도 립싱크 의혹 있던데ㅋㅋ
ㄴ ㅇㅇ(78.12) 팬이지만 이번 앨범은 좀...
```

구현: 작품 가드 YAML에 `댓글반응: true` 설정 → Chief Writer 프롬프트에 "본문 뒤에 댓글 반응 5~10개 삽입" 지시. 현대 연예계 장르 가드에 댓글 스타일 가이드라인(커뮤니티별 말투 차이 등) 포함.

**조연 시점 전환 (POV Switch)**

특정 에피소드에서 주인공이 아닌 조연 시점으로 서술:
...
Stage 0/작품 가드 YAML:
  시점전환:
    ...
    빈도: "5~10화마다 1회"
    허용인물: ["라이벌 A", "히로인 B"]
    금지: "주인공의 내면 노출 금지 (조연 시점에서)"
```

구현: Arc 설계(Stage 2)에서 "이번 에피소드는 조연 B 시점" 지정 → Blueprint에 시점 정보 포함 → Chief Writer가 해당 시점으로 집필. `continuity_inspector`에 "시점 인물이 모를 정보를 서술하지 않았는가" 체크 추가.

**1인칭/3인칭 시점 선택 (Stage 4 설정)**

```
Stage 0/작품 가드 YAML:
  서술시점:
    기본: "1인칭"           # 또는 "3인칭 제한적", "3인칭 전지적"
    화자: "주인공"
    금지패턴:
      - "1인칭인데 주인공이 모르는 정보 서술"     # 1인칭 전지적 오류
      - "다른 캐릭터의 내면 직접 묘사"           # 1인칭 제한 위반
```

구현: Chief Writer 프롬프트에 시점 제약 주입 + 장르 가드에 시점 위반 체크 추가. 위 조연 시점 전환과 결합 가능 — "기본 1인칭이지만, 조연 에피소드는 3인칭 제한적으로 전환".

**Stage 4 출력에 시점 표시** — 현재 원고는 카카오/네이버 포맷으로 출력되는데, 시점 정보가 원고 헤더에 표시되지 않음. 에피소드별로 "시점: 1인칭 (주인공)" 또는 "시점: 3인칭 제한적 (조연 B)" 등을 원고 상단 메타데이터에 포함시켜 PD가 한눈에 확인 가능하게. `stage4_orchestrator`의 원고 포매팅 단계에서 삽입.

> [!TIP]
> 댓글 반응, 시점 전환, 1/3인칭 선택은 모두 **작품 가드 YAML**에 설정 → Stage 2 Arc에 반영 → Stage 4 프롬프트에 주입하는 동일한 파이프라인. 5-B'(작품 가드 외부화) 때 함께 설계 가능.

**사망 캐릭터 부활 방지 (Hard Check)**

3-C 시나리오에 포함되지만, LLM이 가장 자주 실수하는 항목이라 **별도 하드 체크** 분리. `state_tracker_npc`에 `deceased=True` 태그 → 이후 에피소드에서 해당 NPC가 행동/대사로 등장 시 **즉시 REJECT** (회상/과거 장면은 예외). `pre_llm_validator.py`에 정규식 기반 선 검증 가능.

**3-E 기존 기능 대조표 (코드베이스 스캔 결과)**

> 아이디어 중 기존 시스템에 이미 부분/완전 구현된 것을 파악. 후임이 중복 작업 안 하도록.

| 아이디어 | 기존 구현 | 관련 파일 | 남은 작업 |
|---------|----------|----------|----------|
| 멀티 드래프트 비교 | ⚠️ **Arc/Blueprint에만 존재** | `arc_ensemble.py`, `blueprint_ensemble.py` | Stage 4 원고에 확장 필요 |
| 컨텍스트 압축 | ⚠️ **기본 구현 있음** | `context_compression.py` (14KB) | 중요도 가중치 로직 추가 |
| 에피소드 의존성 그래프 | ⚠️ **복선 추적만** | `foreshadow_tracker.py` | 의존성 그래프 확장 |
| 품질 회귀 감지 | ❌ 없음 | `pass_rate_monitor.py` | 에피소드 간 비교 로직 추가 |
| 인간 편집 학습 | ❌ 없음 | `failure_learning.py` | 사용자 diff 채널 추가 |
| 드라이런 모드 | ❌ 없음 | `stage4_orchestrator.py` | `dry_run` 플래그 추가 |
| 온도 자동 조절 | ❌ **하드코딩** | `base_agent.py` (모든 에이전트에서 temperature 고정) | `pass_rate_monitor` 연동 |
| 수정 diff 뷰 | ❌ 없음 | — | 신규 개발 |
| 토큰 예산 추적 | ❌ 없음 | `quality_dashboard.py` | 토큰 메트릭 추가 |
| 프롬프트 A/B 테스트 | ✅ **D-4 완료** | `a2c1fb2` (`director_selections`) | Director 선택 추적/승률 조회 적용 |
| 자기진단 대시보드 | ⚠️ **품질만** | `quality_dashboard.py`, `studio_dashboard.py` | 에이전트/Chain 모니터링 추가 |
| 에피소드 선택적 롤백 | ✅ **D-2 완료** | `project_manager.py` (`auto_backtrack_v35()`) | NPC/문장해시/만족도/WorldState/FactLedger 롤백 반영 |
| 장르 가드 자동 학습 | ✅ **D-3 완료** | `modules/validation/style_guard.py` | anti_ai/forbidden/문장길이 자동 검증 적용 |
| 장편 요약 에이전트 | ⚠️ **기본 압축 있음** | `context_compression.py` | 구조화된 Arc 요약 전용 에이전트 |
| Slack 알림 고도화 | ⚠️ **기본 존재** | `slack_bot.py`, `stage2_orchestrator.py` L58 | 품질 회귀 알림 + 리포트 확장 |
| 병렬 검증 최적화 | ⚠️ **병렬→순차 폴백** | `validation_orchestrator.py` | 의존성 그래프 + 조기 종료 |
| NPC 과잉 등장 경고 | ❌ 없음 | `state_tracker_npc.py` | 신규 NPC/화 카운터 추가 |
| 크로스 에피소드 반복 | ❌ **1화 내만** | `repetition_guard.py`, `pre_director_checklist.py` | DB 인덱싱 + 에피소드 간 비교 |
| 클리프행어 추적 | ⚠️ **호흡 분석 있음** | `pacing_analyzer.py` | 엔딩 패턴 분류 추가 |
| 스크린타임 밸런스 | ⚠️ **NPC 빈도 있음** | `chief_writer.py` (`_get_npc_frequency`) | 단어 수 추적 확장 |
| 대사 화자 명확성 | ❌ 없음 | — | 정규식 체크 신규 개발 |
| Context Caching | ✅ **구현 완료** | `base_agent.py` (L920~1068), `chief_writer.py`, `director_continuity.py` | 추가 에이전트 확장 가능 |
| 댓글/여론 반응 | ❌ 없음 | — | 장르 가드 + 프롬프트 신규 |
| 조연 시점 전환 | ✅ **D-1 완료** | `f0390e8` (POV 활성화 + 시점 체인) | YAML 연동 불필요 (규칙 기반) |
| 1인칭/3인칭 선택 | ✅ **V70 구현됨!** | `pre_llm_validator.py` (V70 시점 일관성), `blueprint_ensemble.py` L312~318 (시점 제약), `bible_extractor.py` L441 (시점 추출) | 작품 가드 YAML에서 설정 가능하게 연동 |
| 대리만족 프레임워크 | ❌ 없음 | `director_auditor.py`, `scoring_validator.py` | Director 심사축 추가 |
| 사망 캐릭터 부활 방지 | ⚠️ **deceased 태그 일부 존재** | `state_tracker_npc.py` (L325, L1208 사망 체크) | 하드 REJECT 룰 + pre_llm 선검증 추가 |

> [!IMPORTANT]
> **이미 있는 것 활용 우선순위**:
> 1. 시점 전환/1·3인칭 (✅ V70 + 프리셋) — YAML 연동만 하면 즉시 사용
> 2. Context Caching (⚠️ 필드 존재) — 활성화만 하면 토큰 절감
> 3. 에피소드 롤백 (✅ `9ba06cd`) — NPC 되감기 완료
> 4. A/B 테스트/선택 추적 (✅ `a2c1fb2`) — Director 선택 추적 완료

---

**3-F. 독자 대리만족 품질 검증 (Reader Satisfaction Framework)**

현재 Director 심사 축: 문체, 연속성, 장르 순수성, 구조.
**빠진 축**: 웹소설 핵심인 **"읽는 재미 = 대리만족"** 검증이 없음.

**5대 만족도 축:**

| # | 축 | 핵심 질문 | 측정 방법 |
|---|---|----------|----------|
| 1 | **대리만족** | 독자가 주인공을 통해 쾌감을 느끼는가? | 에피소드 내 "성취/승리/인정 장면" 빈도 |
| 2 | **보상 체감** | 주인공이 노력에 합당한 보상을 받는가? | "고난 → 보상" 쌍 추적 (보상 없는 고난 3화+ 연속 시 경고) |
| 3 | **유능함** | 주인공이 유능해 보이는가? | "주인공 주도 해결" vs "타인 구출" 비율 |
| 4 | **공감** | 독자가 주인공 감정에 공감하는가? | 내면 묘사 비율 + 감정 전환의 자연스러움 |
| 5 | **좌절-보상 밸런스** | 무능해 보이는 구간이 너무 길지 않은가? | 좌절 시작 → 보상까지의 화수 추적 |

**핵심 규칙:**

```
[절대 규칙]
- 주인공이 무능해 보이는 구간은 최대 3화 이내에 대리만족 보상 필수
- 5화 연속 "고난만 있고 보상 없음" → REJECT
- 주인공이 타인에 의해 구출되는 횟수가 자력 해결의 2배 초과 → 경고

[빈도 가이드]
- 에피소드 5화 중 최소 1회: 명확한 대리만족 장면 (승리/인정/성장 체감)
- Arc 1개(10~15화) 내 최소 1회: 대형 보상 (큰 승리, 레벨업, 복수 달성 등)
- 좌절 구간 진입 시: 다음 1~3화 내 반드시 반전 또는 부분 보상

[장르별 대리만족 유형]
- 무협: 경지 돌파, 강자 제압, 비급 획득, 복수
- 판타지: 레벨업, 레어 아이템, 던전 클리어, 동료 인정
- 요리: 대회 우승, 미식가 감탄, 레시피 완성, 식당 확장
- 연예계: 차트 1위, 팬 반응, 안티 역관광, 업계 인정
```

**구현 방향:**

```
1. Director 심사에 "대리만족 점수" 축 추가:
   - 현재: 문체(30) + 구조(30) + 연속성(20) + 장르(20) = 100점
   - 개선: 문체(25) + 구조(25) + 연속성(20) + 장르(15) + 대리만족(15) = 100점

2. 에피소드별 자동 태깅 (state_extractor 확장):
   - 매화 끝에 자동 분류: [성취] [좌절] [이행] [일상] [전투] [성장]
   - DB에 저장 → 연속 패턴 분석 가능

3. 좌절-보상 타이머:
   - "좌절" 태그 시작 → 카운터 시작
   - 3화 내 "성취/보상" 태그 없으면 → Arc 설계(Stage 2)에 "보상 에피소드 삽입 권장" 알림
   - continuity_inspector에 통합

4. 작품 가드 YAML 연동:
   대리만족설정:
     최대좌절연속: 3        # 화 수
     최소보상빈도: 5        # 화당 1회
     주인공유능비율: 0.6     # 자력해결 / 전체해결 ≥ 60%
     타인구출상한: 2         # Arc당 최대 2회
```

> [!IMPORTANT]
> 이 프레임워크는 Phase 3(검증 강화) + 5-B'(작품 가드 외부화)에서 함께 구현.
> 가장 빠른 적용: Director 프롬프트에 "대리만족 축" 텍스트 추가만으로도 LLM 기반 1차 검증 가능.
...
- stage2: self.app 341건, stage4: 312건 → DI 패턴 전환
- 테스트 시 mock 주입 가능
...
### Phase 5: 프롬프트 외부화 & 설정 관리 `[SAFE]` (5-A ✅, 5-A' ✅, 5-B ✅, 5-C ✅)
...
> [!NOTE]
> ✅ Phase 5-A' 완료 (2026-02-13). 7파일 전부 `PromptLoader.load()` 전환 완료. `rg "_prompts import" modules/domain/agents` → 0건.

**5-B. 설정 관리 + 장르/작품 가드 외부화 `[SAFE]` — 완료 (2026-02-14)**

> 아래 표/설명은 초기 설계 기록이다. 최신 구현 상태 판단은 문서 상단 "2026-02-14 업데이트" 섹션을 우선한다.
...
### [역사 기록] Phase 6: 테스트 & CI (당시 기준: 6-C 부분완료 / 6-A, 6-B 미착수)
...
### 🔴 3. God Object (`self.app` 653건)

| 파일 | self.app 사용 |
|------|--------------|
| **stage2_orchestrator.py** | **341건** |
| **stage4_orchestrator.py** | **312건** |
...
| 3 | ABC/Protocol | Phase 2-C | ✅ 완료 (`d45d53c`) |
...
| **5-A'** | 사용처 import → `PromptLoader.load()` 전환 | `[SAFE]` | ✅ 완료 (2026-02-13) |
| **6-C** | pre-commit + ruff 설정 | `[SAFE]` | ✅ 완료 (2026-02-13) |
| **6-A** | pytest 테스트 도입 (GenreGuard/RepetitionGuard/PromptLoader 63개) | `[SAFE]` | ✅ 완료 (2026-02-13) |
| **2.5/4D** | ChromaDB → sqlite-vec 마이그레이션 + 레거시 제거 | `[RISKY]` | ✅ 완료 |
| **3-5B** | 수정 모드 — Stage 4 패치 모드 | `[RISKY]` | ✅ 완료 |
| **3-5A** | NPC append-only 이력 DB + 검증 파이프라인 통합 | `[RISKY]` | ✅ 완료 (2026-02-14) |
| **5-B** | Settings YAML + ConfigManager 확장 + 임계값 외부화 | `[SAFE]` | ✅ 완료 (2026-02-14) |

### 미완료 Phase (우선순위순, 2026-02-16 기준)

| Phase | 내용 | 안전 등급 | 전제 조건 |
|-------|------|-----------|-----------|
| **4(잔여)** | 코드 리팩토링 잔여 (async 통일/대형 함수 분할) | `[RISKY]` | 6-B |
...
### 완료 메모 (Phase 5-A')
- 사용처 import 전환은 완료됨 (`PromptLoader.load` 기반)
- 세부 치환(`.format_map`) 및 fallback 정리는 후속 유지보수 항목으로만 남김

---

## 🤖 Agent Context (미래 에이전트용 메타 정보)

> **이 섹션은 후임 에이전트가 가장 먼저 읽어야 할 요약입니다.**

### 현재 시스템 상태 요약 (2026-02-14 기준)

- **작동함**: 원고 생성 파이프라인(Stage 0→2→4)은 정상 동작
- **Phase 4D 완료**: ChromaDB 레거시 제거, VecMemory 단일 경로
- **Phase 3-5A 완료**: NPC append-only 이력 DB (`npc_history` 테이블), 사망NPC 회상/행동 구분, 성격급변 감지
- **Phase 5-B 완료**: `validation.yaml` + ConfigManager 확장 + 4개 validator 임계값 외부화 (`threshold_helper.py`)
- **현재 단계**: Phase 4 구조 개선 완료 (R1~R3 분할, R4 NO-GO) → 잔여 테스트 / Phase 3 품질 기능
- **주요 약점**: 플롯 중복 감지 불안정 (Chain 1)
- **미결정**: LangGraph → ❌ 미도입 결정 (기존 오케스트레이터로 충분)
...
| 5-A' | PromptLoader import 전환 | ✅ 완료 |
| 5-B | Settings YAML + 임계값 외부화 | ✅ 완료 (2026-02-14) |

### RISKY 작업 (순서 중요)

> [!IMPORTANT] 아래 표가 현재 기준 실행 순서다. `CLAUDE.md`와 동기화되어야 한다.

| 순서 | Phase | 작업 | 전제 조건 |
|------|-------|------|-----------|
| ~~1~~ | ~~2-A~~ | ~~Pydantic 모델 도입~~ | ✅ 완료 |
| ~~2~~ | ~~2.5 / 4D~~ | ~~sqlite-vec 도입~~ | ✅ 완료 |
| ~~3~~ | ~~3-5B~~ | ~~수정 모드 — Stage 4 패치 모드~~ | ✅ 완료 |
| ~~4~~ | ~~3-5A~~ | ~~NPC 이력 + 검증 강화~~ | ✅ 완료 |
| ~~5~~ | ~~5-B~~ | ~~Settings/가드 외부화~~ | ✅ 완료 |
| ~~6~~ | ~~6-B~~ | ~~E2E 테스트~~ | ✅ 완료 (22 E2E) |
| ~~7~~ | ~~4(잔여)~~ | ~~God Object 분해 + 파일 분할~~ | ✅ R1~R3 완료, R4 NO-GO |
...
| `modules/core/stage2_orchestrator.py` | Arc 오케스트레이터 (2134줄) | God Object, Chain 2 |
| `modules/core/stage4_orchestrator.py` | 원고 오케스트레이터 (1633줄) | 1354줄 단일 try |
| `modules/domain/agents/*.py` | AI 에이전트 20+개 | |
| `modules/core/genre_guards/*.py` | 장르 가드 3개 | |
| `config/settings/validation.yaml` | 검증 임계값 설정 (5-B) | |
| `modules/validation/threshold_helper.py` | 공유 `_threshold()` 헬퍼 (5-B-2c) | |
| `참고자료.md` (이 문서) | 전체 컨텍스트 종합 참고자료 | **후임 필독** |
...
### 📅 현실적 작업 일정 (Claude Code Pro Max $200/월 기준)

> 각 Phase는 **세션 단위** 작업. 한도 초과 대비 Opus/Sonnet 혼합 운영.
> 실작업은 ~20시간이면 끝나지만, **쿼터 한도에 의해 주 단위로 분산됨.**

```
주 1~6: [완료] 6-C/6-A/5-A'/2-A/2-C + 4D(sqlite-vec)         ← 완료
주 7:   [완료] 3-5B 수정 모드 (Stage4 패치 모드)               ← 완료
주 8:   [완료] 3-5A NPC 이력 + 검증 강화                       ← 완료 (2026-02-14)
주 8:   [완료] 5-B 설정/가드 외부화                            ← 완료 (2026-02-14)
주 9:   [O] 6-B E2E 테스트                                     ← 다음
주 10~: [O] Phase 4 잔여 구조 개선                              ← 장기
```

**쿼터 운영 전략:**
- [O] 작업은 **TF(계획서)를 먼저 짜고**, 실행은 최대한 [S]로 돌림
- 한도 터지면 → [S] 작업 먼저 소화 (import 전환, pytest 등)
- 대형 [O] 작업은 주 초에 시작 → 한도 리셋 전 마무리
- 전체 로드맵 소화: **약 3개월**
