...
1. **Python은 수집만, 판단은 LLM이** — Python 코드는 데이터 수집·포맷팅·전달만 수행. "이게 오류인가?", "수정해야 하는가?" 같은 판단은 반드시 LLM 에이전트가 담당.
2. **팩트시트 수정 권한은 LLM만** — NPC 속성, 세계관 설정, 관계도 등 팩트시트를 수정할 수 있는 건 LLM 에이전트뿐. Python 로직이 자동으로 팩트를 덮어쓰면 안 됨.
3. **사망 캐릭터는 회상/언급은 가능** — `deceased=True`인 NPC가 행동/대사로 등장하면 REJECT, 단 회상·과거 장면·타인의 언급은 허용.

### 🤖 모델 역할 분담

| 역할 | 모델 | 용도 |
|------|------|------|
| 창작·판단 | **Opus 4.6** (고급) | Arc 설계, 원고 집필, Director 심사, 연속성 판단 |
| 단순 작업 | **Sonnet 4.5** (경량) | 디버깅, state_extractor 추출, 포맷 변환, 반복 검증 |

> 비용 최적화: 전체 토큰의 ~60%가 Chief Writer + Director에 집중 → 이 부분만 Opus, 나머지는 Sonnet으로 분리.
