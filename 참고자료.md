...
Phase 2:   타입 시스템 도입  ← 2-B 완료
           ├─ 2-A: Pydantic 도입 (dict → 모델 전환) ← 미착수
           ├─ 2-B: Type Hints 보강 (78% → 95.5%) ✅
           └─ 2-C: ABC/Protocol 에이전트 인터페이스 ← 미착수
  ↓
...
Phase 3:   검증 & 안정화 ← 미착수
           ...
           ├─ 3-B: Silent pass 59건 정리 (B등급 보류분)
           ├─ 3-C: NPC/세계관 연속성 실패 시나리오 24개 대응
           │       (사망 캐릭터 부활 방지 하드 체크 포함)
           ├─ 3-D: 수정 모드 (전체 재작성 → 부분 수정 전환)
           ├─ 3-E: 개선 아이디어 27개 (대조표 포함)
           │       ├─ 멀티 드래프트, 드라이런, 온도 자동조절
           │       ├─ 토큰 예산 추적, A/B 테스트 고도화
           │       ├─ 댓글 반응 생성, 시점 전환, 1/3인칭 선택
           │       ├─ Context Caching, 에피소드 롤백 강화
           │       └─ 기존 기능 대조표 (구현 현황 매핑)
           └─ 3-F: 독자 대리만족 프레임워크 (5대 만족도 축)
  ↓
Phase 4:   구조 개선 ← 미착수
...
Phase 5:   프롬프트 외부화 & 설정 관리
           ├─ 5-A: 프롬프트 외부화 (43개 YAML) ✅
           ├─ 5-A': 사용처 import 전환 (40+파일) ← 미착수
           ├─ 5-B: Settings YAML 통합 + 장르/작품 가드 외부화 ← 미착수
           │       (댓글 반응, 시점 설정, 대리만족 설정 등 통합)
           └─ 5-C: 의존성 정리 (requirements.txt) ✅
  ↓
Phase 6:   테스트 & CI
           ├─ 6-A: 단위 테스트 (pytest, 핵심 모듈) ← 미착수
           ├─ 6-B: 통합 테스트 (E2E 파이프라인) ← 미착수
           └─ 6-C: 코드 품질 (ruff ✅, pre-commit 미설정)
```
