...
- **현재 단계**: **Phase 3-5B 구현중** — Stage 4 점수 기반 패치 모드 (`docs/phase3_5b_blueprint.md`)
- **다음 우선순위**: 3-5B(수정 모드) → 3-5A(NPC 이력) → 5-B(설정 외부화) → 6-B(E2E) → 4(잔여)
- 주의: 아래 일부 표/다이어그램은 과거 시점 수치를 포함하며, 최신 상태는 본 섹션을 우선한다.

### 🔒 Phase 3-5B 정책 고정 블록

> 아래 정책은 Phase 3-5B 구현 기간 중 절대 변경 금지. 상세: `docs/phase3_5b_blueprint.md`

1. 후보 수 3개 유지 (패치 모드도 앙상블 3후보)
...
| 4 | **Phase 3-5B (수정 모드: Stage4 패치 모드)** | **🚧 현재 진행** | ★★★☆☆ |
| 5 | Phase 3-5A (NPC 이력 + 검증 강화) | ⏳ 다음 | ★★★★☆ |
| 6 | Phase 5-B (Settings/가드 외부화) | ⏳ 대기 | ★★★☆☆ |
| 7 | Phase 6-B (E2E 테스트) | ⏳ 대기 | ★★★★☆ |
...
> | 3-5B (수정 모드) | `RISKY` | 4D 완료 ✅ | 🚧 진행중 | `stage4_orchestrator.py`, `chief_writer.py`, `chief_writer.yaml` |
> | 3-5A (NPC 이력/검증) | `RISKY` | 3-5B | 대기 | `state_tracker*`, 검증 계열 모듈 |
...
> | 5-B (설정 관리) | `SAFE` | 3-5A 이후 권장 | 대기 | `config_manager.py`, `.env` |
...
| ~~**2.5 / 4D**~~ | ~~벡터 DB 교체 + 벡터 경로 재연결~~ | `[RISKY]` | ✅ **완료** (Phase 4D) |
| **3-5B** | **수정 모드 — Stage 4 패치 모드** (`docs/phase3_5b_blueprint.md`) | `[RISKY]` | 4D ✅ ← **현재** |
| **3-5A** | NPC 이력 + 검증 강화 | `[RISKY]` | 3-5B |
| **5-B** | Settings 관리 + 장르/작품 가드 외부화 | `[SAFE]` | 3-5A |
| **6-B** | E2E 테스트 | `[RISKY]` | 5-B |
...
- **현재 단계**: Phase 3-5B 구현중 (Stage 4 패치 모드)
- **주요 약점**: NPC 연속성 추적이 약함 (3-C 시나리오 24개 참조), 플롯 중복 감지 불안정 (Chain 1)
...
| 5-A' | PromptLoader import 전환 (함수형 프롬프트 포함, 40+파일) | ✅ 완료 |
| 5-B | Settings YAML 통합 + 장르/작품 가드 외부화 | 대기 (3-5A 이후 권장) |

### RISKY 작업 (순서 중요)

> [!IMPORTANT] 아래 표가 현재 기준 실행 순서다. `CLAUDE.md`, `내일작업.md`와 동기화되어야 한다.

| 순서 | Phase | 작업 | 전제 조건 |
|------|-------|------|-----------|
| ~~1~~ | ~~2-A~~ | ~~Pydantic 모델 도입~~ | ✅ 완료 |
| ~~2~~ | ~~2.5 / 4D~~ | ~~sqlite-vec 도입~~ | ✅ 완료 |
| 3 | **3-5B** | **수정 모드 — Stage 4 패치 모드** | 4D ✅ ← **현재** |
| 4 | 3-5A | NPC 이력 + 검증 강화 | 3-5B |
| 5 | 5-B | Settings/가드 외부화 | 3-5A |
| 6 | 6-B | E2E 테스트 | 5-B |
...
| `modules/core/genre_guards/*.py` | 장르 가드 3개 | 외부화 예정 (5-B) |
...
```
주 1~6: [완료] 6-C/6-A/5-A'/2-A/2-C + 4D(sqlite-vec)         ← 완료 이력
주 7:   [O] 3-5B 수정 모드 (Stage4 패치 모드)                  ← 현재
주 8:   [O] 3-5A NPC 이력 + 검증 강화                          ← 다음
주 9:   [S/O] 5-B 설정/가드 외부화                             ← 후속
주 10:  [O] 6-B E2E 테스트                                     ← 안정화
주 11~: [O] Phase 4 잔여 구조 개선                              ← 장기
```
