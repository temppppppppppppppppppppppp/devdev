...
- **현재 단계**: **Phase 3-5B 설계중** — Stage 4 점수 기반 패치 모드 (`docs/phase3_5b_blueprint.md`)
- **다음 우선순위**: 5B(수정 모드) → 5A(NPC 이력) → 6B(E2E)
...
Phase 1.5: 레거시 정리  ← 부분 완료                     ★☆☆☆☆
           (전부 [S] — 삭제/이동)
  ↓
Phase 2:   타입 시스템 도입  ← 2-B 완료                 ★★★☆☆
           ├─ 2-A: Pydantic 도입 ← 미착수     [O]      ★★★☆☆
           │       (모델 설계 + 전체 에이전트 연쇄 수정)
           ├─ 2-B: Type Hints 95.5% ✅        [S]      ★★☆☆☆
           └─ 2-C: ABC/Protocol 인터페이스     [O]      ★★☆☆☆
  ↓
Phase 2.5: DB 통합 (SSOT) ← 미착수                     ★★★★☆
           ├─ sqlite-vec 도입                  [O]      ★★★★☆
           │       (임베딩 마이그레이션 + API 교체)
           ├─ JSON → 구조화 칼럼               [O]      ★★★☆☆
           └─ DB 통합                          [S]      ★★☆☆☆
  ↓
Phase 3:   검증 & 안정화 ← 미착수                       ★★★★☆
           ├─ 3-A: ValidationError 버그 수정   [S]      ★★☆☆☆
           ├─ 3-B: Silent pass 59건 정리       [S]      ★★☆☆☆
           ├─ 3-C: NPC 연속성 24개 대응        [O]      ★★★★☆
           │       (이력 테이블 설계 + 검증 로직)
           ├─ 3-D: 수정 모드                   [O]      ★★★☆☆
           │       (오케스트레이터 루프 재설계)
           ├─ 3-E: 아이디어 27개               [S~O]    ★~★★★★
...
Phase 4:   구조 개선 ← 미착수                           ★★★★★
           ├─ God Object 분해 (→ DI)           [O]      ★★★★★
           │       (653건 self.app 의존성 분석 필수)
           ├─ 대형 파일 분할                    [O]      ★★★★☆
           └─ async/sync 통일                  [S]      ★★★☆☆
  ↓
Phase 5:   프롬프트 외부화 & 설정 관리                   ★★☆☆☆
           ├─ 5-A: 외부화 ✅                   [S]      ★★☆☆☆
           ├─ 5-A': import 전환 (40+파일)      [S]      ★★☆☆☆
           │       (기계적 치환 — grep+sed 수준)
           ├─ 5-B: YAML + 가드 외부화          [O]      ★★★☆☆
           │       (스키마 설계 + 파이프라인 연동)
           └─ 5-C: 의존성 정리 ✅              [S]      ★☆☆☆☆
...
| **3-5A** | NPC 이력 + 검증 강화 | `[RISKY]` | 5B |
| **5-B** | Settings 관리 + 장르/작품 가드 외부화 | `[SAFE]` | 없음 |
| **6-B** | E2E 테스트 | `[RISKY]` | 5A |
...
- **현재 단계**: Phase 3-5B 설계중 (Stage 4 패치 모드)
...
| 5-A' | PromptLoader import 전환 (함수형 프롬프트 포함, 40+파일) | 미착수 |
| 5-B | Settings YAML 통합 + 장르/작품 가드 외부화 | 미착수 |

### RISKY 작업 (순서 중요)

> [!NOTE] **최신 우선순위는 `CLAUDE.md` 및 `내일작업.md` 참조.** 아래는 초기 계획으로, 실제 진행 순서와 다를 수 있음.
...
| 4 | 3-5A | NPC 이력 + 검증 강화 | 5B |
| 5 | 4(잔여) | God Object 분해 + 파일 분할 | E2E 이후 |
...
```
주 1: [S] 6-C pre-commit ✅ + 6-A pytest ✅ + 5-A' import 전환     ← 진행중
주 2: [O] 2-A Pydantic 설계 + 1차 구현                         ← Opus TF
주 3: [O] 2-A 마무리 + [S] 3-A/3-B 버그 정리                   ← 혼합
주 4: [O] 3-C NPC 연속성 (이력 테이블 + 검증)                   ← Opus
주 5: [O] 3-D 수정 모드 + 3-F 대리만족                         ← Phase 3-5B로 재편 (docs/phase3_5b_blueprint.md)
주 6: [O] 2.5 sqlite-vec                                     ← ✅ Phase 4D로 완료
주 7~8: [O] 5-B 가드 외부화 + [O] 6-B E2E 테스트              ← 혼합
주 9~12: [O] Phase 4 God Object 분해                          ← 최난관
```
