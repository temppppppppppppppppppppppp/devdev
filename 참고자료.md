...
- **현재 단계**: Phase 4 구조 개선 완료 → 잔여 테스트 커버리지 / Phase 3 품질 기능
- **완료**: 6-B(E2E 22), 4-R1~R3(몬스터 분할), R4-a(NO-GO)
- **테스트 기준선**: unit 29 + pipeline 89 + E2E 22 + regression 78 = **218 passed** (checkpoint `a8f5bee`)
...
> | 6-B (E2E 테스트) | `RISKY` | 3-5A | 대기 | 파이프라인 전체 |
...
| 프롬프트 A/B 테스트 | ⚠️ **기본 존재** | `ab_testing.py` (`quick_ab_test()`) | 자동 채택 로직 확장 |
| 자기진단 대시보드 | ⚠️ **품질만** | `quality_dashboard.py`, `studio_dashboard.py` | 에이전트/Chain 모니터링 추가 |
| 에피소드 선택적 롤백 | ⚠️ **기본 존재** | `project_manager.py` (`auto_backtrack_v35()`) | NPC 상태 되감기 + 의존성 경고 |
| 장르 가드 자동 학습 | ⚠️ **문체 분석 존재** | `stage0/style_extractor.py` (`StyleExtractor`) | 어휘 빈도 → 가드 초안 자동 생성 |
...
| 조연 시점 전환 | ✅ **프리셋 존재!** | `blueprint_ensemble.py` L81~84 (`villain_scheme`, `side_glimpse`, `omniscient_hint`) | 작품 가드 YAML 연동만 필요 |
...
> 3. 에피소드 롤백 (⚠️ `auto_backtrack_v35`) — NPC 되감기만 추가
> 4. A/B 테스트 (⚠️ `quick_ab_test`) — 자동 채택 로직만 추가
...
### 미완료 Phase (우선순위순, 2026-02-14 기준)

| Phase | 내용 | 안전 등급 | 전제 조건 |
|-------|------|-----------|-----------|
| **6-B** | **E2E 테스트** | `[RISKY]` | 5-B ✅ ← **다음** |
