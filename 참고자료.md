...
flowchart TB
    ...
    subgraph S2["⚙️ Stage 2: Arc 설계"]
        S2O["stage2_orchestrator\n2142줄 (God Object)"]
        S2O --> WV["weaver\n욕망 드라이브"]
    ...
    subgraph S4["✍️ Stage 4: 원고 집필"]
        S4O["stage4_orchestrator\n1633줄"]
        S4O --> CW["chief_writer\n2115줄"]
        S4O --> CR["critic\n원고 비평"]
    ...
    style S2O fill:#ff6b6b,color:#fff
    style S4O fill:#ff6b6b,color:#fff
    style CW fill:#ffa94d,color:#fff
    style GEMINI fill:#00b894,color:#fff
    style SQL fill:#6c5ce7,color:#fff
    style YAML fill:#00cec9,color:#fff
```
...
sequenceDiagram
    actor PD as PD님
    participant UI as main_a.py
    participant S0 as Stage 0
    participant DB as SQLite DB
    participant S2 as Stage 2 Orch
    participant AG as Agents
    participant LLM as Gemini API
    participant S4 as Stage 4 Orch
    ...
    rect rgb(255, 230, 230)
        Note over S2: Stage 2: Arc 설계 (반복 루프)
        UI->>S2: Arc 생성 시작
        S2->>DB: 이전 Arc/상태 로드
...
graph LR
    ...
    style APP fill:#ff6b6b,color:#fff,stroke:#c0392b,stroke-width:3px
