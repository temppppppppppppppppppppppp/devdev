...
**Gemini Context Caching 활용**

Gemini API에 Context Caching 기능 존재. 매 에피소드마다 반복되는 세계관 바이블 + 캐릭터 프로필을 캐싱 → **토큰 비용 최대 50% 절감**. `base_agent.py`에 이미 `_context_caches` 필드가 있지만 활용 안 되고 있음 → 활성화만 하면 즉시 효과.
...
| Context Caching | ⚠️ **필드만 존재** | `base_agent.py` (`_context_caches` L881) | 활성화 + Gemini API 연동 |
