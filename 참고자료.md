...
>
> | Phase | 안전 레벨 | 선행 조건 | 핵심 파일 |
> |-------|----------|----------|----------|
> | 1, 1.5 | `SAFE` ✅ | 없음 | `base_agent.py`, `chief_writer.py`, `stage2_orchestrator.py`, `db_manager.py` |
> | 2-B (Type Hints) | `SAFE` ✅ | 없음 | `modules/` 전체 |
> | 2-A (Pydantic) | `RISKY` | Phase 1 완료 | `domain/models.py`, 에이전트 전체 |
> | 2-C (ABC/Protocol) | `RISKY` | Phase 2-A 완료 | `base_agent.py`, 에이전트 전체 |
> | 2.5 (DB 통합) | `RISKY` | Phase 2 완료 | `memory_engine.py`, `db_manager.py`, `blueprint_memory.py` |
> | 3 (검증) | `RISKY` | Phase 2 완료 | StateTracker, Director 등 핵심 에이전트 |
> | 4 (구조 개선) | `RISKY` | Phase 3 완료 | `main_a.py`(4138줄), `stage2`(2133줄), `stage4`(1632줄) |
> | 5-A (프롬프트 외부화) | `SAFE` ✅ | 없음 | `manager.py`, `writer.py`, `director.py` 등 에이전트 |
> | 5-B (설정 관리) | `SAFE` ✅ | 없음 | `config_manager.py`, `.env` |
> | 5-C (의존성 정리) | `SAFE` ✅ | 없음 | `requirements.txt` |
> | 6-A (단위 테스트) | `SAFE` ✅ | 없음 | 새 파일 생성 (`tests/`) |
> | 6-B (E2E 테스트) | `RISKY` | Phase 2-A 완료 | 파이프라인 전체 |
> | 6-C (코드 품질) | `SAFE` ✅ | 없음 | `pyproject.toml`, `.pre-commit-config.yaml` |
>
> **안전 작업 우선 순서** (Phase 번호와 무관하게 절약 가능):
> `5-A (프롬프트 외부화)` → `5-B (설정 관리)` → `5-C (의존성)` → `6-A (단위테스트)` → `6-C (linter)`
...
### Phase 2.5: sqlite-vec 도입 (ChromaDB → SQLite 통합)
...
- Phase 1에서 B등급으로 보류했던 핵심 로직 silent pass
- Pydantic 모델 도입 후 안전하게 `ValidationError`로 전환 가능
...
### Phase 5: 프롬프트 외부화 & 설정 관리 `[SAFE]`

**현재 문제:**

- 에이전트 파일들(`manager.py`, `writer.py`, `director.py` 등)에 수백 줄짜리 프롬프트가 Python 문자열로 하드코딩
- 프롬프트 수정할 때마다 Python 코드를 건드려야 함 → 실수 유발
- 모델명(`gemini-1.5-pro` 등)이 여러 파일에 산재
- `.env` + 코드 내 하드코딩이 혼재하여 환경별 전환 불가

**5-A. 프롬프트 외부화 `[SAFE]` — 런타임 변경 0**

- `config/prompts/` 디렉토리에 YAML/JSON으로 분리
- 에이전트별 1파일: `architect.yaml`, `director.yaml`, `chief_writer.yaml` 등
- Python 코드에서는 `PromptLoader.load("director", "audit")` 식으로 호출
- 장점: 프롬프트 A/B 테스트, 버전 관리, 비개발자도 수정 가능

**5-B. 설정 관리 체계화 `[SAFE]` — config 파일 추가만 함**
...
- `config/genres/wuxia.yaml`: 장르별 설정 (금기어, 필수 묘사 등)

**5-C. 의존성 정리 `[SAFE]`**

- `requirements.txt` 버전 핀닝 (`google-genai==1.x.x` 등)
- 불필요 의존성 제거 (비활성화된 ChromaDB 등)
- `requirements-dev.txt` 분리 (pytest, linter 등 개발 전용)

---

### Phase 6: 테스트 & CI `[6-A, 6-C: SAFE / 6-B: RISKY]`
...
**6-C. 코드 품질 자동화 `[SAFE]` — 설정 파일 추가만 함**

- `pre-commit` 훅: ruff (linter) + black (formatter)
- `.pylintrc` / `pyproject.toml` 프로젝트 표준 설정
...
```
Phase 1:   except Exception 디버깅
Phase 1.5: 레거시 제거 (writer, DEAD CODE)
Phase 2:   Pydantic 도입
Phase 2.5: sqlite-vec 도입 ← 여기 (ChromaDB import 제거 + 벡터 통합)
Phase 3:   ValidationError 추가 수정
```

Phase 2(Pydantic) 이후가 적절 — Pydantic 모델로 데이터 구조가 정리된 상태에서 벡터 칼럼 추가가 더 안전.
...
### 🔴 1. print() 2289건 vs logging 76건

| 파일 | print 건수 |
|------|-----------|
| main_a.py | 134 |
| stage2_orchestrator.py | 103 |
| spinner.py | 68 |
| stage0/__init__.py | 65 |
| validation_orchestrator.py | 58 |

> [!CAUTION]
> **print()는 레벨/필터링/파일 출력 제어 불가**. 디버그 로그와 사용자 UI 출력이 섞여 있어, 에러 추적 시 유용한 로그만 필터링할 수 없음.
>
> **개선**: Python `logging` 모듈 도입 → `DEBUG/INFO/WARNING/ERROR` 레벨 분리. 이미 `modules/core/logger.py`가 존재하므로 이를 확장.

### 🟡 2. Type Hints 52% (1400/2685 함수)

- 반환 타입 힌트 있음: **52%**
- 없는 함수: **48%** (1285개)

Pydantic 도입(Phase 2) 시 자연스럽게 개선되지만, 핵심 함수부터 `-> Optional[dict]` 등 추가 권장.
...
| 순위 | 항목 | Phase | 근거 |
|------|------|-------|------|
| 1 | print → logging | Phase 1과 함께 | silent pass 수정 시 자연스럽게 |
| 2 | Type Hints | Phase 2와 함께 | Pydantic 도입과 동시에 |
| 3 | ABC/Protocol | Phase 2와 함께 | Pydantic 모델 정의 시 함께 |
| 4 | JSON-in-SQLite | Phase 2.5와 함께 | DB 통합 시 스키마 정리 |
| 5 | God Object 분해 | Phase 3 이후 | 장기 아키텍처 과제 |
| 6 | 파일 분할 | Phase 3 이후 | 안정화 후 |
| 7 | async 통일 | Phase 3 이후 | 기능에 영향 없으므로 후순위 |
