...
- **다음 우선순위**: **Phase 4D (기존 로드맵 2.5)** — sqlite-vec 기반 SSOT 통합 + 벡터 저장/검색 경로 재연결
...
| **2.5 / 4D** | 벡터 DB 교체 + 벡터 경로 재연결 (ChromaDB→sqlite-vec) | `[RISKY]` | Phase 4C 완료 ✅ |
| **3** | 검증 및 버그 픽스 | `[RISKY]` | 2 전체 |
| **4(잔여)** | 코드 리팩토링 잔여 (async 통일/대형 함수 분할) | `[RISKY]` | 3 |
...
| 순서 | Phase | 작업 | 전제 조건 |
|------|-------|------|-----------|
| 1 | 2-A | Pydantic 모델 도입 | 2-B 완료 ✅ |
| 2 | 2.5 | sqlite-vec 도입 (ChromaDB 교체) | 2-A 완료 |
| 3 | 3 | NPC 이력 테이블 + 관계 그래프 + 수정 모드 | 2-A 완료 |
| 4 | 4 | God Object 분해 + 파일 분할 | 3 완료 |
...
주 5: [O] 3-D 수정 모드 + 3-F 대리만족                         ← Opus
주 6: [O] 2.5 sqlite-vec                                     ← Opus
