...
**3-D. 원고 수정 모드 (Revision Mode) — 전체 재작성 → 부분 수정 전환**

현재 `stage4_orchestrator`의 Director 면담 루프:

```
Chief Writer 원고 작성 → Director 심사
  → 불합격: 피드백 + 통째로 다시 써 (최대 3회)
  → 3회 실패: 냉동인간(Writer) 폴백
```

**문제**: 80점짜리 원고도 통째로 버리고 다시 씀 → 다시 쓴 게 60점일 수도 있고, 토큰도 낭비.

**개선안: 점수 기반 분기**

```
Director 심사 결과:
  < 50점: 전체 재작성 (현재와 동일 — 구조적으로 망한 원고)
  50~80점: "수정 모드" — 기존 원고 + 수정 지시만 전달, 부분 수정
  > 80점: 통과
```

| | 전체 재작성 | 수정 모드 |
|--|-----------|----------|
| 토큰 비용 | 전체 원고 분량 | 수정 지시 + 수정 대상 부분만 |
| 좋은 부분 보존 | ❌ 날아감 | ✅ 유지 |
| 연속성 위험 | 🔴 새로 쓰면서 앞뒤 틀릴 수 있음 | 🟢 기존 맥락 유지 |
| 품질 안정성 | 🎲 운빨 | ✅ 점진적 개선 |
...
위 지시에 따라 원고를 수정하되, 지시받지 않은 부분은 절대 변경하지 마라.
```

> [!TIP]
> `stage4_orchestrator.py`의 `stage_4_v2_chief_writer()` 메서드에서 Director 면담 루프(Phase 4)를 수정하면 구현 가능. Phase 3(검증 강화) 때 함께 진행.
