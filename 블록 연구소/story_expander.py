...
try:
    from utils.spinner import (
        Spinner, ProgressBar, PhaseIndicator,
        print_header, print_success, print_error, print_warning, print_info, print_tip, print_table
    )
...
"""
    ...
    def phase1_generate_bible(self) -> dict:
        ...
        events = self.genre.get_historical_events(
            self.extracted.get("timeline", {}).get("start_era", "")
        )
...
"""
    ...
    def _generate_world_state(self) -> dict:
        ...
        timeline = self.extracted.get("timeline", {})
        return {
            "CurrentEra": timeline.get("start_era", ""),
            "CurrentLocation": "시작 위치",
            "KarmaMatrix": {}
        }
    ...
    def _generate_treatment_skeleton(self) -> list:
        ...
        if hasattr(self.advisor, 'get_arc_recommendations'):
            ...
            for arc in range(1, 11):
                ...
                arc_recs.append({
                    "arc": arc,
                    "blocks": f"{block_num}-{block_num+5}",
                    "advice": advice.get("overall_tip", "")
                })
...
JSON만.
"""
            response = self._call_llm(prompt, temperature=0.85, spinner_msg=f"Block {start_block}-{start_block + len(batch) - 1} 생성")
...
```
"""
            response = self._call_llm(prompt, temperature=0.3, max_tokens=1000, spinner_msg=f"메타 {i+1}/{len(blocks)}")
