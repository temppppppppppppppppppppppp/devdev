...
def perform_selective_rewind(target_ep, db_path, drafts_path):
    ...
    try:
        ...
        # 5. [DB-MERGE] project_data.db 내부 벡터 테이블 리셋
        try:
            rows = cursor.execute("SELECT ep_num FROM episode_meta WHERE ep_num >= ?", (target_ep,)).fetchall()
            for (ep,) in rows:
                cursor.execute("DELETE FROM vec_episodes WHERE rowid = ?", (ep,))
            cursor.execute("DELETE FROM episode_meta WHERE ep_num >= ?", (target_ep,))
            cursor.execute("UPDATE sync_status SET vector_synced = 0 WHERE ep_num >= ?", (target_ep,))
            conn.commit()
            print(f"   🔆 벡터 메모리 제거 완료 ({len(rows)}건)")
        except Exception as vdb_err:
            print(f"   ⚠️ 벡터 테이블 제거 건너뜀: {vdb_err}")
