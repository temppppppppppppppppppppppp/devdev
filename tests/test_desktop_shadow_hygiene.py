...
ROOT_PRELOAD = (ROOT / "geuldobi-desktop/preload.js").read_text(encoding="utf-8")
...
def test_root_main_is_only_a_compatibility_shim():
    ...
    assert "Compatibility preload shim only." in ROOT_PRELOAD
    assert "Authoritative preload entry: geuldobi-desktop/src/preload.js" in ROOT_PRELOAD
    assert 'module.exports = require("./src/preload.js");' in ROOT_PRELOAD
...
def test_dead_candidate_ipc_surfaces_are_split_from_live_inventory():
    ...
    assert "getWorkspacePath" not in ROOT_PRELOAD
    assert "workspace:get-path" not in ROOT_PRELOAD
    assert "getWorkspacePath" not in PRELOAD
    assert "workspace:get-path" not in CONTROL_PLANE_JS
    assert "workspace:get-path" not in LIVE_MAIN
