...
@pytest.mark.parametrize("bad_sub", ["7", "8", "99", "a", "-1"])
...
@pytest.mark.parametrize("key", ["1", "2", "3", "4", "5", "6", "44", "77", "88", "99"])
...
@pytest.mark.parametrize("key", ["1", "2", "4", "6"])
def test_running_state_blocks_new_run(key: str) -> None:
    result = validate_run_request(key=key, sub_key=None, runner_state="running")
    assert not result.ok
    assert result.http_status == 409
    assert result.code == "RUN_ALREADY_ACTIVE"


def test_key0_running_state_also_blocked() -> None:
    result = validate_run_request(key="0", sub_key="1", runner_state="running")
    assert not result.ok
    assert result.code == "RUN_ALREADY_ACTIVE"
...
@pytest.mark.parametrize("sub_key", ["0", "1", "2", "3", "4", "5", "6"])
...
def test_non_running_states_do_not_block() -> None:
    for state in ("idle", "waiting_input", "stopping", "error"):
