...
def test_plan_stage4_retrieval_passes_current_ep(tmp_path):
    ...
    assert "manuscript_excerpt" in slot_names, f"manuscript_excerpt 슬롯 미생성 (current_ep=5). 슬롯: {slot_names}"
...
def test_fetch_manuscript_snippet_returns_excerpt(tmp_path):
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS manuscripts (ep_num INTEGER PRIMARY KEY, title TEXT, content TEXT)")
    conn.execute(
    ...
    snippet = vm.fetch_manuscript_snippet(10, max_chars=30)
...
def test_fetch_manuscript_snippet_safe_on_missing(tmp_path):
    ...
    snippet = vm.fetch_manuscript_snippet(999)
    assert snippet == ""
...
def test_fetch_manuscript_snippet_safe_when_no_table(tmp_path):
    ...
    snippet = vm.fetch_manuscript_snippet(1)
    assert snippet == ""
