"""Prepare, run, and analyze a repeatable Stage 3-only canary project.

Validation tier: full_canary_proof
Mutation boundary: boots live app surfaces, patches input, and writes proof artifacts.
Stage 4 bypass: Stage 4 is never invoked. The script calls
``app._stage_3_batch_blueprinting()`` directly via the Stage3Orchestrator,
then closes the app without entering any Stage 4 method.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage 3-only canary helper")
    subparsers = parser.add_subparsers(dest="command", required=True)

    prepare = subparsers.add_parser("prepare", help="copy a baseline project and reset Stage 3 outputs only")
    ...
    run = subparsers.add_parser("run", help="run Stage 3 only on a prepared project")
    run.add_argument("--project", required=True)
    run.add_argument("--target-ep", type=int, default=4)

    analyze = subparsers.add_parser("analyze", help="analyze Stage 3 canary results")
...
def prepare_canary(source_project: str, target_project: str, *, from_ep: int, force: bool) -> dict:
    ...
    return prepare_stage3_canary_project(source_root, target_root, from_ep=from_ep, force=force)
...
def run_canary(project_name: str, *, target_ep: int) -> dict:
    ...
    try:
        ...
        with patch("builtins.input", side_effect=_auto_input):
            _run_stage3_only(app, target_ep=target_ep)
...
def _run_stage3_only(app: SovereignApp, *, target_ep: int) -> dict:
    """Invoke Stage 3 batch blueprinting directly, bypassing Stage 4 entirely."""
...
def analyze_canary(project_name: str, *, target_ep: int) -> dict:
    project_root = resolve_workspace_project_dir(PROJECT_ROOT, project_name, prefer_canary=True, require_exists=True)
    summary = build_stage3_canary_summary(project_root, target_ep=target_ep)
