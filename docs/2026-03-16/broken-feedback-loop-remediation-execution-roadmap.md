...
- the low-priority `control-plane provenance` reader is also closed via bridge `/status`, `PassRateMonitor.get_patch_effectiveness()` and `QualityDashboard.get_quality_signal_snapshot()` are now closed via bridge quality dashboard, and `HUD anomaly` / `blueprint coverage` producers are now closed in QualityDashboard
...
- no residual low-priority helper or instrumentation items remain in this roadmap lane; a future validity gate may reopen the roadmap only if new drift is discovered
