...
- the low-priority `control-plane provenance` reader is also closed via bridge `/status`, `PassRateMonitor.get_patch_effectiveness()` is now closed via bridge quality dashboard, and `HUD anomaly` / `blueprint coverage` producers are now closed in QualityDashboard; only opportunistic helper cleanups remain
...
- residual low-priority items are now outside the remediation queue unless a future validity gate promotes them; `PassRateMonitor.get_patch_effectiveness()` and the `HUD anomaly` / `blueprint coverage` producer gap no longer belong to that residual set
