...
- 우측: 대기/결과 연출 패널(모래성 빌드 + 상태 이펙트)

## 상태/이펙트 규칙
- `RUNNING`: 아이가 모래성 기초/벽체를 쌓는 루프
- `PASS`: 완성 모래성 + 깃발/빛 효과
- `REJECT`: 파도/바람으로 일부 붕괴 후 정지
- `PASS_WITH_FIX`: 탑/성벽 다듬기 연출 + `"다듬는 중"` 배지
...
모래성 연출 샘플
1. `RUNNING`: 바닥 다지기/삽질 루프 + "심사중..."
2. `PASS`: 완성 성채 실루엣 + 금빛 하이라이트
3. `PASS_WITH_FIX`: 벽체 보강/장식 추가 + "다듬는 중"
4. `REJECT`: 파도 1회로 일부 붕괴 + 즉시 로그 복귀
