...
### v1.18 메뉴 재구조화 (2단 카테고리)

기존 flat 버튼 목록을 **비개발자 친화적 2단 카테고리**로 개편한다.
...
│              │ Office 뷰 + 에이전트      │ 실시간 stdout/WS   │
│ ▶ 재료 넣기   │ 상태 애니메이션            │ verdict/score 뱃지 │
│   Bible 설정  │ PASS/REJECT/PWF 이펙트     │ 검색/필터/복사      │
│   Treatment  │ (Skip/Mute 가능)          │                  │
│   역설계      │                            │                  │
│   스타일 분석  │                            │                  │
│              │                            │                  │
│ ▶ 상품 생산   │                            │                  │
...
| 카테고리 | UI 라벨 | 콘솔 키 | 비고 |
|----------|---------|---------|------|
| **재료 넣기** | Bible 설정 | `0` → sub_key `1` | 기존 방식(Bible/Treatment 파일 선택) |
| | Treatment 확장 | `0` → sub_key `5` | Block 확장 |
| | 역설계 | `0` → sub_key `3` | 기존 원고에서 Bible/스타일 추출 |
| | 스타일 분석 | `0` → sub_key `6` | 스타일 레퍼런스 분석 |
| | 컨셉 → Bible | `0` → sub_key `2` | AI 컨셉 확장 (`STAGE0_AVAILABLE` 필요) |
| | Bible JSON 임포트 | `0` → sub_key `4` | JSON 파일 임포트 (`STAGE0_AVAILABLE` 필요) |
| **상품 생산** | 장르설정 | (UI 전용) | 장르 선택 모달, 백엔드 키 없음 |
| | ~~Volume Strategy~~ | ~~`1`~~ | **숨김 처리** (코드 보존, UI 미노출) |
| | Arc 설계 | `2` | Stage 2 |
| | Blueprint | `3` | Stage 3 |
| | 원고 생산 | `4` | Stage 4 |
| | One-Stop | `6` | Arc-by-Arc 자동 실행 |
| **운영** | Rollback | `44` | Stage 4 회차 롤백 (위험키) |
| | Wipe | `77` | Stage 4 삭제 (위험키) |
| | Reset | `88` | Stage 2 초기화 (위험키) |
| | Rewind | `99` | Stage 2 되감기 (위험키) |
| | Stop | `stop` | 실행 중지 |

### 장르설정 모달 명세

"상품 생산" 카테고리 진입 시, **프로젝트에 장르가 미설정이면** 장르설정 모달을 먼저 표시한다.
이미 설정된 경우 건너뛰고, 설정 변경은 "장르설정" 버튼 직접 클릭으로만 가능.

| 장르 | UI 표시 | 상태 |
|------|---------|------|
| 투자물 | 투자물 | 활성 |
| 무협 | 무협 (테스트 없음) | 선택 가능, 경고 표시 |
| 헌터 | 헌터물 (테스트 없음) | 선택 가능, 경고 표시 |
| 판타지 | 판타지 (테스트 없음) | 선택 가능, 경고 표시 |
| 의료 | 의료물 (테스트 없음) | 선택 가능, 경고 표시 |
| 대체역사 | 대체역사 (테스트 없음) | 선택 가능, 경고 표시 |
| 작곡 | 작곡물 (테스트 없음) | 선택 가능, 경고 표시 |
| 스포츠 | 스포츠물 (테스트 없음) | 선택 가능, 경고 표시 |
| 배우 | 배우/연예물 (테스트 없음) | 선택 가능, 경고 표시 |
| 요리 | 요리물 (테스트 없음) | 선택 가능, 경고 표시 |

디자인 규칙:
- "(테스트 없음)" 라벨은 `color: #94a3b8` 회색 + `font-size: 0.8em` 축소
- 투자물 외 장르 선택 시 확인 모달: "이 장르는 실파이프라인 테스트가 완료되지 않았습니다. 진행하시겠습니까?"
- 장르 선택값은 로컬 프로젝트 설정에 저장 (`project_config.genre`)
- 백엔드 전달: Bible 내 장르 필드와 동기화 (Stage 0에서 설정된 값과 충돌 시 UI 선택 우선)

### Stage 1 숨김 정책

| 레이어 | 처리 | 상세 |
|--------|------|------|
| **프론트엔드** | `display: none` | Stage 1 버튼에 `.hidden-stage` 클래스 추가. 개발자 도구에서 해제 가능 |
| **백엔드 API** | 키 유지 | `ALLOWED_KEYS`에 `"1"` 잔류. CLI 경로 정상 작동 |
| **main_a.py** | 코드 보존 | `_stage_1_volumes()` 삭제 안 함. 콘솔 메뉴에서 `1` 입력 시 정상 실행 |
| **One-Stop** | 영향 없음 | Stage 1을 건너뛰고 Stage 2→3→4 실행 (기존 동작 동일) |

숨김 이유:
- Stage 1(Volume Strategy)은 Arc 2~3개 단위 권 전략 설계로, 현 PoC 범위에서 UI 노출 시 혼란 유발
- CLI 개발자 경로는 유지하되, 비개발자 UI에서는 불필요한 선택지를 제거하여 UX 단순화

### 변경 영향도 분석

| 영역 | 변경 필요 | 규모 |
|------|----------|------|
| `index.html` 실행 패널 | ✅ HTML 재구조화 | 중 (카테고리 접기/펼치기 + 장르 모달) |
| `index.html` CSS | ✅ 카테고리 스타일 | 소 |
| `index.html` JS | ✅ 카테고리 토글 + 장르 모달 로직 | 중 |
| `bridge_server.py` | ❌ 변경 없음 | - |
| `run_validator.py` | ❌ 변경 없음 | - |
| `main_a.py` | ❌ 변경 없음 | - |
| `process_runner.py` | ❌ 변경 없음 | - |
| API 계약 | ❌ 변경 없음 | - |
| 기존 테스트 | ❌ 영향 없음 | - |
...
### 개요
현재 UI에 설정 버튼은 존재하나 **기능 미구현**(스텁). 아래 명세를 기준으로 구현한다.
설정은 **모달 또는 사이드 패널**로 제공하며, 변경 즉시 `.env`/YAML에 반영한다.
...
| 항목 | UI 컴포넌트 | 저장 위치 | 비고 |
|------|------------|----------|------|
| Gemini API Key (기본) | 비밀번호 입력 필드 + 표시/숨김 토글 | `.env` `GOOGLE_API_KEY` | **필수** — 미입력 시 앱 실행 불가 |
| 추가 API Key 2~9 | 접기/펼치기 + 번호 부여 입력 필드 | `.env` `GOOGLE_API_KEY_2`~`9` | 선택 — 쿼터 분산용 |
| Slack Webhook URL | 텍스트 입력 필드 | `.env` `SLACK_WEBHOOK_URL` | 선택 — 알림 연동 |

동작 규칙:
- API 키는 `*` 마스킹 기본, "보기" 버튼으로 토글
- 저장 시 `.env` 파일 직접 갱신 (`dotenv` 포맷 유지)
- 키 변경 후 "테스트" 버튼: `genai.GenerativeModel.generate_content("test")` 1회 호출로 유효성 확인
- 키가 비어 있으면 상단 경고 배너: "API 키를 설정해주세요"
- 키 순환 현황 표시: "현재 사용 중: Key 1 / 총 3개 등록"

#### 탭 2: 모델 설정

| 항목 | UI 컴포넌트 | 저장 위치 | 비고 |
|------|------------|----------|------|
| 에이전트별 모델 | 드롭다운 (Pro/Flash) | `config/models.yaml` | 고급 설정 |
| 폴백 체인 | 읽기 전용 표시 | `config/models.yaml` | 참고용 |

동작 규칙:
- 기본 모드: "추천 설정" 1버튼 (models.yaml 기본값 적용)
- 고급 모드: 에이전트별 모델 선택 (chief_writer, director, analyst 등)
- 변경 시 재시작 필요 안내
...
| 항목 | UI 컴포넌트 | 저장 위치 | 비고 |
|------|------------|----------|------|
| 현재 장르 | 읽기 전용 + 변경 버튼 | 프로젝트 DB | 장르설정 모달 재호출 |
| 작가 지시사항 | 텍스트에어리어 | `{project}/config/author_directives.txt` | 선택 |
| 작품 가드 YAML | 파일 업로드/편집 | `{project}/work_guard.yaml` | 선택 — 작품별 금기어 등 |

#### 탭 4: 시스템 (고급)

| 항목 | UI 컴포넌트 | 저장 위치 | 비고 |
|------|------------|----------|------|
| API 타임아웃 | 슬라이더 (60~600초) | `config/system.yaml` | 기본 300초 |
| 키 순환 간격 | 슬라이더 (5~60초) | `config/system.yaml` | 기본 10초 |
| 품질 게이트 점수 | 숫자 입력 (50~100) | `config/settings/validation.yaml` | 기본 90, 전문가 전용 |
| 원고 목표 길이 | 숫자 입력 | `config/settings/validation.yaml` | 기본 5000자 |
| 연출 설정 | Skip animation / Mute 토글 | 로컬 설정 | 앱 재시작 불필요 |
...
앱 최초 실행
  ↓
[1] API 키 입력 화면 (필수, 스킵 불가)
  ↓ 키 테스트 통과
[2] 프로젝트 생성/선택
  ↓
[3] 장르 선택 (상품 생산 첫 진입 시)
  ↓
메인 화면 진입
```
...
| 변수명 | 필수 | 용도 | 기본값 |
|--------|------|------|--------|
| `GOOGLE_API_KEY` | **필수** | Gemini API 기본 키 | 없음 (앱 실행 불가) |
| `GOOGLE_API_KEY_2`~`9` | 선택 | 쿼터 분산용 추가 키 | 없음 (단일 키 모드) |
| `SLACK_WEBHOOK_URL` | 선택 | Slack 알림 웹훅 | 없음 (알림 비활성) |
| `STAGE0_AVAILABLE` | 선택 | Stage 0 확장 기능 활성화 | `false` |
| `PYTHONIOENCODING` | 자동 | Windows UTF-8 콘솔 | `utf-8` (자동 설정) |

### 설정 파일 의존성 맵

```text
.env (API 키)                        ← 설정 탭 1에서 편집
config/models.yaml (모델 배정)        ← 설정 탭 2에서 편집
config/system.yaml (타임아웃/캐시)     ← 설정 탭 4에서 편집
config/settings/validation.yaml (임계값) ← 설정 탭 4에서 편집
config/genres/{name}.yaml (장르 규칙)  ← 읽기 전용 (장르 선택 시 자동 적용)
config/prompts/*.yaml (프롬프트)       ← UI 미노출 (수정 금지)
{project}/config/author_directives.txt ← 설정 탭 3에서 편집
```
...
- `v1.18` 메뉴 재구조화: flat→2단 카테고리(재료넣기/상품생산/운영) + 장르설정 모달 + Stage 1 숨김 + 설정 패널 명세(API키/모델/프로젝트/시스템) + 환경변수 전체 목록 + 첫 실행 온보딩 플로우
