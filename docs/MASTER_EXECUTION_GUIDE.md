...
## 11. Stage 2 에너지 하드코딩 감리 (2026-02-25) — NO-GO 판정

### 11-1. 발견 사항

Stage 2 감리에서 `internal_energy=100` 하드코딩 7개소 발견.

### 11-2. 판정: 수정 불필요 (전량 NO-GO)

`final_energy = 100`은 **LLM 에스컬레이션 방지 안전장치**로 의도된 설계.

**문제가 되는 시나리오 (안전장치 없을 때)**:
```
Arc 1: 내공 100% → 전투 → 80%
Arc 2: 내공  80% → 전투 → 50%
Arc 3: 내공  50% → 전투 → 20% → 주인공 폐인화
```

**안전장치 작동 (현재)**:
```
Arc 1: 내공 100% → 전투 → 80% → Python 리셋 → 100%
Arc 2: 내공 100% → 전투 → 70% → Python 리셋 → 100%
```

LLM은 부상/에너지 저하를 매 Arc마다 누적 강화하는 경향 → Python 레벨 강제 리셋이 프롬프트와 이중으로 방어.

### 11-3. 비무협 장르에서 무해한 이유

1. `internal_energy`는 Stage 3/4 Python 로직에서 사용 안 함 (순수 LLM 컨텍스트)
2. LLM은 `_NON_WUXIA_ENERGY_BLOCK`의 "internal_energy=0 (해당없음)" 지시를 따름
3. 비무협 장르에서 실질 장애 0건 (실제 운영 데이터 확인)

### 11-4. 요약

| 항목 | 판정 | 이유 |
|------|------|------|
| 에너지 하드코딩 7개소 | **NO-GO** | 의도된 안전장치, LLM 에스컬레이션 방지 |
| SSOT shadow 폴백 5건 | **후순위 관찰** | Pydantic validator가 방어 중, 실질 발동 없음 |

> **주의**: 이 하드코딩을 제거하면 주인공 점진적 약화(폐인화) 버그가 재발한다. 건드리지 말 것.
