...
- 외부 보고용 공식 단계명은 Production Pilot이 적절하다. 즉 실제 산출물 생산은 가능하지만, 정식 상업 운영 전 검증과 고도화가 남아 있는 제한 생산 파일럿 단계다.
- 내부 진단은 축별로 보는 것이 정확하다. S2/S3는 안정화 후기, S4는 세션메모리와 컨텍스트 캐싱을 통해 최적화 중기 진입, 재료 사이드는 도너 구조와 BI/TR 표준화까지 포함한 생산 체계 구축 단계다.
...
글도비의 외부 보고용 공식 단계명은 `Production Pilot`이다. 한국어로는 `상업 적용 전 제한 생산 파일럿`이라고 설명할 수 있다. 이는 PoC나 Prototype이 아니라 실제 심사 원고 생산은 가능한 단계이며, 정식 상업 운영 전 pass율, 재시도율, 장기기억, 장르별 안정성을 검증하는 구간이라는 뜻이다.

다만 내부 진단은 한 단계로만 묶기보다, 원고 런타임과 재료 사이드를 분리해서 보는 편이 정확하다.
...
운영 방식에 대한 오해도 분리할 필요가 있다. 글도비는 사람이 중간에 원고를 직접 고쳐 넣는 편집기가 아니라, DB와 컨텍스트 누적, JSON/dict 기반 handoff를 통해 단계별 산출물을 넘기는 생산 파이프라인이다. 기본 흐름은 `추상화 -> 구체화`의 일방향 구조이며, 기획안, Phase 0, TR, BI, work_guard 같은 재료를 먼저 만든 뒤 S2, S3, S4를 거쳐 원고로 내려간다.

따라서 사람이 중간에 전개를 직접 바꾸거나, 외부에서 직접 만든 15화를 그대로 글도비 내부 재료로 되돌리는 `구체화 -> 추상화` 방식은 현재 기본 설계 대상이 아니다. 사람의 개입 지점은 supervised run에서 결과를 검토하고, 실패 원인을 분류하고, 다음 run의 지시와 재료를 보강하는 방식에 가깝다. 외부 원고를 다시 시스템에 태우는 역방향 import나 편집기형 워크플로는 별도 기능이며, 현재 파일럿 범위로 보는 것은 부정확하다.
...
- "글도비는 중간 편집기가 아니라, 추상 재료를 구체 원고로 내려보내는 handoff 기반 생산 파이프라인이다."
