...
Date: 2026-04-20
Status: active operator audit
Scope: root `treatments/` + `bible/` lane labeling for `TR/BI` pair work

## 1. Purpose

- root `treatments/` and `bible/` are mixed live inventories, not one clean pair shelf
- this audit labels the current lanes without moving live artifacts
...
- do not move the current mixed root files in this wave; keep using manifest + labeling first
