...
|----|------|----------|-----------|-----------|-----------|--------|---------|
| S1-EX-001 | ~~`.gitattributes` 파일 생성 및 CRLF 정책 확립~~ | ~~P0~~ **DONE** | `.gitattributes` 파일이 부재하여 Windows 환경에서 LF/CRLF 혼재 발생. `git diff` 시 CRLF 경고 8건 확인. `*.py text eol=lf`, `*.js text eol=lf`, `*.html text eol=lf`, `*.yaml text eol=lf`, `*.json text eol=lf` 등 주요 확장자별 정책을 명시하고, `.editorconfig`에 `end_of_line = lf` 추가 (현재 `.editorconfig` `[*]` 블록 line 3-5에 `charset`/`insert_final_newline`만 존재 → line 5 `insert_final_newline = true` 다음에 `end_of_line = lf` 삽입). | (1) `.gitattributes` 파일 존재 (2) `git diff` 실행 시 CRLF 경고 0건 (3) `.editorconfig`에 `end_of_line = lf` 명시 (4) CI 또는 pre-commit hook에서 line-ending 검사 통과 | 0.5h | 없음 | S1 §10.1: ".gitattributes 부재 → CRLF 경고 8건"; S1 §8: ".editorconfig에 end_of_line 미지정" |
| S1-EX-002 | ~~CRLF 혼재 파일 일괄 정규화~~ | ~~P0~~ **DONE** | `.gitattributes` 적용 후 기존 파일의 line ending을 일괄 정규화. `git add --renormalize .` 실행 후 커밋. 경고 발생 파일 8건을 특정하여 변환 확인. CRLF 파일 열거 명령: `git diff --check HEAD~1 2>/dev/null | grep "trailing whitespace\|CRLF" | head -20` | (1) `git grep -I -l $'\r'` 결과 0건 (2) 정규화 커밋 완료 (3) 이후 `git diff` 시 CRLF 관련 경고 0건 | 0.5h | S1-EX-001 | S1 §10.1: "CRLF 경고 8건"; S1 §8: "Windows LF/CRLF 혼재 가능" |
