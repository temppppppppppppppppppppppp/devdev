...
| G3 | 시뮬레이션 도구 가격 갱신 | **P1** | `cost_calculation.py`, `full_project_cost.py`의 가격이 런타임 대비 1/2~1/4 과소. Flash input $0.075->$0.30 (4x), Flash output $0.30->$2.50 (8.3x), Pro output $5->$10 (2x). **빌드업 비용 테이블 완료**: `docs/2026-03-18/buildup-cost-table-rol-metric-definition.md` (3회 재조사 + 적대적 3-pass). Thinking 토큰은 output에 이미 합산 확정, Claude/GPT 가격 테이블 포함 | 시뮬 도구 MODEL_COSTS가 `metrics_collector.py`와 동일 가격 사용 | 0.25일 | 없음 | S7 섹션 4.1, 딥다이브 섹션 5.2 발견 3, **buildup 비용 테이블** |
