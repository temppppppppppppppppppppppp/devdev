...
| EX-10 | Verdict 상태 체계 정리: 9 Core + 7 Contextual → 스키마 반영 (S1) | **P2-Medium** | **빌드업 매핑표 완료**: `docs/2026-03-18/buildup-verdict-6way-mapping-table.md` (3회 재조사 + 적대적 3-pass). 실제 verdict 9개(PASS, PASS_WITH_FIX, REJECT, CONDITIONAL_PASS, PASS_WITH_WARNING, FAILED, ERROR, EMPTY, SKIP) + contextual decision 7개(CONFLICT, WARNING, RETRY, AGREE, DISMISS, UNKNOWN, FAIL). 스키마 3개 vs 코드 16개. 부울 플래그 7개(quality_risk, quality_gate_failed, force_reject, force_pass_with_fix, asp_used, patch_used, patch_fallback)도 문서화 완료. | `response_schemas.py`에 PASS_WITH_WARNING을 정식 등록하거나, 별도 verdict enum 모듈 생성하여 단일 진실 소스 확보. SQL WHERE절이 enum 참조하도록 변경. CONDITIONAL_PASS no-op 여부 문서화. | 5h (스키마 정의 + SQL 정리 + 하류 영향 분석 + 테스트) | 없음 | S6 \S4.1, S1 |
