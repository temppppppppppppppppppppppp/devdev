...
- **Material-side §4.3.1 Structured Ref Convention Alias 적용** (ratified 2026-04-09): 본 work은 `foreshadow: [{ref: int, event: str}, ...]` / `callback: [{ref: int, event: str}, ...]` convention을 채택했으므로 `material_ssot/00_governance/production-pair-schema-standard-v1.md §4.3.1`의 structured ref convention alias가 적용된다. content.*/stakes/power_shift.*/relationship_delta/foreshadow.event/callback.event/strategy/success_pattern 필드의 `B숫자`/`ARC-숫자` inline 참조는 TR-내부 plan-level 참조로 허용되며, 감리 보고서에서 `meta_number_leak_blocks` 수치는 INFO-level 관찰 항목으로 분류한다. 권한 체인: material-side §4.3.1 → wuxguide 하네스 §0D Rule 22A (family-specific 구현 노트, subordinate). Precedents: `docs/2026-04-09/manual_meridian_archivist_b46_b47_3pass_audit.md` §2.5, `docs/2026-04-09/manual_meridian_archivist_b48_3pass_audit.md` §4.3.
- **Material-side Tier A compliance** (2026-04-09 복구 완료): `production-pair-schema-standard-v1.md §4.6` wuxguide Tier A 11 surfaces 전수(48/48) PASS. B39~B48 `genre_ext.enemy_pressure` 10 블록 빈 문자열 drift 복구 완료. top-level `_draft_status: active_production` 추가. Phase0 `_phase0_ref` / `_work_id` / `_authority_chain` / `_family` 전량 canonical write. Alias matrix(§8) drift 0. `martial_ext`는 read-compat alias로 유지, canonical forward write는 `genre_ext`.
