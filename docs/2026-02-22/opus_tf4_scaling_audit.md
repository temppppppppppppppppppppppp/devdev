...
1. `get_manuscripts_range(start_ep, end_ep)` 배치 쿼리 메서드 추가:
   ...
   ORDER BY ep_num ASC
   ```
2. **3단계 하이브리드 전환** (30화 전문 대체):
   - **1단계**: 최근 10화 전문 (2 Arc 분량, 직접 연속성 보장)
   - **2단계**: 11~30화 전 화별 요약 (500자/화, `get_recent_manuscript_excerpts()` 활용)
   - **3단계**: 31화~ 이전 전체 Arc 단위 요약 (1,000자/Arc)
   - 200화 시 예상: 150,000자 → ~94,000자 (**-37%**)
   - 50화 시 예상: 150,000자 → ~64,000자 (**-57%**)
3. **SC(Smart Context Retrieval) 연동**: 31화 이전 디테일이 필요할 때 벡터 검색으로 정밀 검색
   - 현재 30화 전문은 31화 이전 정보를 완전히 소실하지만, 3단계+SC는 전체 이력 검색 가능
4. 결과를 세션 내 캐시 (dict[int, str])에 보관하여 Director 심사 시 재로드 방지
...
1. **30화 전문 → 3단계 하이브리드로 전환** (가장 큰 개선 효과):
   ...
   - **절감 효과**: 150,000자 → 94,000자 (**-37%**, 200화 기준)
   - 10화 전문 = 2개 Arc → 직전 Arc의 대사/묘사/감정 변화까지 참조 가능
   - 현재 30화 전문은 31화 이전 정보를 **완전히 소실**하지만, 3단계는 전체 이력 커버

2. **SC(Smart Context Retrieval) 연동**: Arc 요약만으로 부족한 디테일은 벡터 검색으로 온디맨드 정밀 검색
   - 예: 150화에서 42화 복선 회수 시 → Arc 요약에서 위치 파악 → SC가 42화 원문 관련 단락 검색

3. state_tracker 16종 요약에 총량 상한 추가 (예: 15,000자)
4. `resolved_plots_summary`에 최대 출력 항목 수 제한
...
| 순위 | 대상 | 등급 | 조치 | 예상 효과 |
|------|------|------|------|-----------|
| **1** | 30화 전문 로드 (1-1) | BOTTLENECK | 3단계 하이브리드 (10화 전문 + 20화 요약 + Arc 요약) + SC 검색 | Context 37~57% 절감 + 전체 이력 검색 가능 |
