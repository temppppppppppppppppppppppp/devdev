# Philosophy To WorkGuard Translation Map

Historical trace note:

- this dated working doc is superseded by `material_ssot/20_pitch/work-guard-translation-map.md`
- use the `material_ssot` path as the canonical authority

- Date: 2026-04-06
- Scope: no-code downstream translation guide
- Purpose: convert the canonical material-side philosophy into a runtime-safe `work_guard.yaml` shape
- Upstream law source:
...
## 4. Constitution -> WorkGuard Quick Map
...
- Pass 1: `work_guard`가 실제로 소비하는 필드와 철학 조항을 맞춰 봄
- Pass 2: 장문 철학을 넣지 말아야 하는 이유와 compact 예산을 다시 점검함
- Pass 3: 사람이 바로 적용할 수 있게 예시 YAML과 operator recipe를 추가함
- Confidence: 0.98
