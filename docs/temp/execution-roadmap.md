...
2. `0_0-stage234-global-authority-alignment-bounded-remediation` (new bounded cross-stage follow-up lane; after `Stage3 Tranche C`, the current local operator preference now chooses one shared `CrossStageAuthorityPacket` transport lane across `Stage2 emit -> Stage3 consume -> Stage4 consume/persist` ahead of rerun reopening or retry-owner-debt structure work, and this lane stays narrower than the older broad Stage234 vocabulary substrate)
