...
| 순서 | 기능 | 전제 | 상태 |
|------|------|------|------|
| ~~다음~~ | ~~C. NPC 과잉 등장 경고~~ | ~~A 완료 후 번들~~ | ✅ 완료 (`409093c`) — extra-only advisory, core NPC(KeyNPCs) 제외, longest-match masking, 테스트 12개 |
| 다음 | B. 크로스 에피소드 반복 | A+C 완료 후 (DB 인덱싱 필요) | 대기 |
| 장기 | D. 대리만족 프레임워크 | A~C 안정 후 (Director 심사축 변경) | 대기 |
