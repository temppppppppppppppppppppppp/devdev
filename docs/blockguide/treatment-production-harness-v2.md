...
```
1. Bible 준비 (세계관, 주인공, 장르 설정)
   ↓
2. Phase 0: 대단원 아크 설계 (§2)
   → 적대자 변천, NPC 타임라인, 자본 곡선, 복선 맵, 패배 계획
   ↓
3. Phase 1: 배치 생성 (§3)
   → 모델별 배치 크기 (Opus 10 / Pro 5 / Flash 3)
   → 사전 선언 + 절대 금지 규칙 + Anti-Shortcut Harness
   ↓
4. Phase 2: Python 자동 교정 (§4)
   → 자본/NPC/pov/is_regressor 강제 교정
   ↓
5. Phase 3: Python 자동 검증 + 위반 블록 재생성 (§5)
   → validate_v1() + validate_v2() = 16개 패턴
   → 위반 블록만 LLM 재생성
   ↓
6. Phase 4: 3-Pass 감리 (§6)
   → 1차 전수 → 2차 오탐 제거 → 3차 최종 확정
   ↓
7. 출고 게이트 통과 (§7)
   → P0 0건, P1 0건, P2 권장 충족
   ↓
8. treatments/{작품명}_tr_block_070_draft.json 저장
```
