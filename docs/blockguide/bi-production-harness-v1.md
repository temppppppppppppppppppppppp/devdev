...
2. `treatment-production-harness-v2.md`를 다시 확인해 TR이 감리 통과 상태인지 본다.
...
BI 단계도 기본적으로 사용자 승인 없이 아래 순서대로 자동 전진한다.
사용자가 따로 멈추지 않는 한, `TR draft`를 SSOT로 삼아 생성과 감리를 끝까지 이어 간다.
...
1. auto-run은 기본값이다. 사용자가 수동 검토를 명시한 경우에만 멈춘다.
2. auto-run이어도 `BI 생성 -> 감리 -> 정리`를 한 번에 뭉개지 않고 단계별로 기록한다.
3. 컨텍스트 compaction이 발생해도 auto-run 기본값은 유지된다. 이 경우 `TR draft`, `phase0_design`, 기존 `BI`를 UTF-8로 재오픈한 뒤 자동 재개한다.
4. BI와 감리 보고서는 전부 **UTF-8 only**로 읽고 쓴다. 한글 깨짐은 즉시 재생성 대상으로 본다.
5. BI 감리 PASS 전에는 후처리 삭제로 넘어가지 않는다.
6. 중간에 하나라도 실패하면 자동 진행을 중단하고, 실패 단계와 원인을 바로 보고한다.

### 3.3 초세분화 handoff 루틴 (저지능 LLM 기본값)

느리더라도 아래 12단계를 그대로 따르면 BI 오염 가능성이 가장 낮다.

1. `blockguide-integrated-order.md`를 다시 읽는다.
2. `phase0_design`를 UTF-8로 연다.
3. `tr_block_070_draft`를 UTF-8로 연다.
4. 기존 `0_bi_{work_id}.json`이 있으면 UTF-8 파싱과 오염 여부만 확인한다.
5. 오염이 있으면 부분 수선보다 재생성을 택한다.
6. BI 최소 스켈레톤만 먼저 만든다.
7. `plot_roadmap`는 `TR draft`에서 복사 주입한다.
8. 장기 복선, NPC 타임라인, 자본 곡선은 `phase0_design`에서만 복사한다.
9. 직접 새로 쓰는 한글은 상단 짧은 메타 필드로 제한한다.
10. UTF-8로 저장한다.
11. 정합성 검증과 5-Pass 감리를 실행한다.
12. PASS가 뜬 뒤에만 handoff 완료로 기록한다.
