...
def load_manuscripts(input_path: str) -> list[dict]:
    ...
    if path.is_file():
        ...
        files = sorted([f for f in path.glob("*.txt") if "합본" not in f.name], key=lambda f: f.name)
...
에피소드는 연속된 숫자여야 합니다 (예: [5,6,7] O, [5,7,9] X)"""
    ...
    if "blocks" not in result:
        ...
        for arc in arcs:
            blocks.append(
                {"block_id": arc["arc_id"], "episodes": arc["episodes"], "title_hint": f"아크 {arc['arc_id']}"}
            )
        return blocks
...
def extract_all_treatments(llm: TreatmentLLM, blocks: list[dict], manuscripts: list[dict]) -> list[dict]:
    ...
    for i, block in enumerate(blocks):
        ep_range = (
            f"{block['episodes'][0]}-{block['episodes'][-1]}"
            if len(block["episodes"]) > 1
            else str(block["episodes"][0])
        )
...
def main():
    ...
    parser.add_argument(
        "--arc-size", "-a", type=int, default=DEFAULT_ARC_SIZE, help=f"아크 크기 (기본: {DEFAULT_ARC_SIZE})"
    )
    ...
    print("🚀 Treatment Extractor v2.0 시작")
    ...
    print(
        f"   블록당 에피소드: 평균 {sum(ep_counts) / len(ep_counts):.1f}개, 최소 {min(ep_counts)}개, 최대 {max(ep_counts)}개"
    )
