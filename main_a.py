...
class SovereignApp:
    ...
    def _phase_0_recovery(self):
        ...
        if p0_choice == "0":
            ...
            self._stage_0_extended(mode=5)  # 스타일 레퍼런스 분석
            return
    ...
    def _stage_0_extended(self, mode: int = 0):
        ...
        - 스타일 레퍼런스 분석 → 참조 원고에서 문체 DNA 추출 (mode=5)
        - mode=0: 메뉴 표시
        """
        ...
        if choice == 1:
            ...
            if style_guide:
                ...
                try:
                    if self.current_project:
                        ...
                        self.current_project.save_v20_anchor("style_guide", sg_data)
            ...
            # [V70] 스타일 레퍼런스 분석 → 참조 원고에서 문체 DNA 추출
            style_guide = stage0_manager.run_reference_analysis()
            if style_guide and self.current_project:
                try:
                    sg_data = json.loads(style_guide.to_json()) if hasattr(style_guide, "to_json") else {}
                    self.current_project.save_v20_anchor("style_guide", sg_data)
    ...
    def _stage_1_volumes(self):
        ...
        for vol_idx in range(1, total_volumes + 1):
            ...
            def _vol_attempt_func(attempt, _feedback):
                """[V65] 단일 권 설계 시도 로직"""
                self.ui.log(
                    f"   {Emojis.BRAIN} 제 {vol_idx}권 전략 설계 중... (시도 {attempt + 1}/{RetryLimits.DIRECTOR_MAX_ATTEMPTS})"
                )
            ...
            def _vol_on_success(vol_data):
                ...
                if not vol_data or not isinstance(vol_data, dict):
                    ...
                    self._audit_event(
                        "analyst_error", "invalid volume data", {"vol_no": vol_idx, "type": str(type(vol_data))}
                    )
                    return False
            ...
            summary = raw_doc[:500]  # [V70] raw_doc은 이미 dict→str 정규화 완료 (line 2472-2474)
    ...
    def _stage_3_batch_blueprinting(self) -> None:
        ...
        if not hasattr(self, "state_tracker") or self.state_tracker is None:
            ...
            if self.state_tracker.npc_registry:
                ...
                self.ui.log(
                    f"      👤 [V60.96] StateTracker 초기화: NPC {len(self.state_tracker.npc_registry)}명 (사망: {dead_count}명)"
                )
        ...
        if not hasattr(self, "world_state") or self.world_state is None:
            try:
                ...
                if _ws_ep > 0:
                    ...
                    self.ui.log("      🌍 [V68] WorldStateManager 초기화 (신규)")
        ...
        if production_head > 0:
            ...
            self.ui.log("📂 [Fresh Start] 기존 데이터 없음 - 1화부터 시작")
        ...
        self.ui.log(f"\n{'═' * 60}")
        self.ui.log("🎯 [V60.80] Three Phase Blueprint Generator 시작")
        ...
        while working_ep <= target_ep:
                ...
                try:
                    if "state_extractor" in self.agents and self.current_project.arcs:
                        ...
                        if all_arcs_for_entity:
                            state_for_entity = self.agents["state_extractor"].extract_cumulative_state(
                                all_arcs_for_entity
                            )
            ...
            try:
                ...
                sys.stderr.flush()
                self.ui.log("      ⚠️ 직전 Blueprint 로드 실패, None으로 진행")
            ...
            try:
                ...
                traceback.print_exc(file=sys.stderr)
                sys.stderr.flush()
                self.ui.log("      ⚠️ 주인공 이름 추출 실패, 기본값 사용")
            ...
            try:
                ...
                with StageSpinner(3, f"제{working_ep}화"):
                    ...
                    for _ms_ep in range(max(1, working_ep - 30), working_ep):
                        _ms_data = self.current_project.db.get_manuscript(_ms_ep)
                        if _ms_data:
                            _ms_text = (
                                _ms_data.get("content", "") if isinstance(_ms_data, dict) else str(_ms_data)
                            )  # [V70] get_manuscript()은 dict 반환
                            if _ms_text:
        ...
        # 통계 출력
        self.ui.log(f"\n{'═' * 60}")
        self.ui.log("📊 [V60.80] Stage 3 완료 통계")
