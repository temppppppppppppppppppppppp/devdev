...
class StageSpinner:
    ...
    def _render(self) -> Text:
        ...
        base_color = self.theme.get("base_color", "cyan")
        ...
        bar_chars = "▰▱░▒▓█"
        for i in range(12):
            idx = (i + self._wave_offset) % len(bar_chars)
            wave_bar += bar_chars[idx % 2]

        detail = f" · {self.task_detail}" if self.task_detail else ""

        # 파도치는 텍스트 조합
        result = Text()
        result.append(f" {frame} ", style=f"bold {base_color}")
        result.append_text(self._wave_text(f"Stage {self.stage}: {stage_name}"))
        result.append(" │ ", style="dim")
        result.append_text(self._wave_text(verb))
        result.append(detail, style="dim white")
        result.append(" │ ", style="dim")
        result.append_text(self._wave_text(wave_bar))
        result.append(f" │ 전체: {self._format_time(session_elapsed)} · 현재: {self._format_time(task_elapsed)}", style="dim")
