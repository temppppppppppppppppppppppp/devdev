...
class FailureLearner:
    ...
    CATEGORY_PATTERNS = {
        ...
        FailureCategory.TIMELINE_ERROR: [r"타임라인", r"시간.*순서", r"시간선", r"timeline", r"수여.*전.*소지"],
        FailureCategory.SCOPE_OVERFLOW: [r"범위.*초과", r"scope.*overflow", r"분량.*초과", r"blueprint.*범위"],
        FailureCategory.BLUEPRINT_MISMATCH: [r"blueprint.*미반영", r"설계.*불일치", r"씬.*누락", r"핵심.*씬.*없"],
        FailureCategory.MISSING_SCENE: [r"필수.*씬", r"장면.*누락", r"missing.*scene", r"cliffhanger.*없"],
        FailureCategory.RELATIONSHIP_JUMP: [r"관계.*급변", r"relationship.*jump", r"무시.*충성", r"적대.*우호.*급변"],
        FailureCategory.CHARACTER_OOC: [r"캐릭터.*일탈", r"out.*character", r"성격.*불일치", r"OOC"],
        FailureCategory.VILLAIN_STUPIDITY: [r"악역.*지능", r"villain.*stupid", r"과소평가", r"악당.*멍청"],
        FailureCategory.FREE_POWERUP: [r"공짜.*파워", r"대가.*없", r"free.*power", r"성장.*정당화"],
        FailureCategory.DEUS_EX_MACHINA: [r"기계신", r"deus.*ex", r"뜬금없", r"복선.*없.*등장"],
        FailureCategory.PACING_ISSUE: [r"호흡", r"pacing", r"템포", r"전개.*속도"],
        FailureCategory.JSON_ERROR: [r"json", r"파싱", r"parsing", r"구조.*오류"],
        FailureCategory.LENGTH_ERROR: [r"길이", r"분량", r"length", r"자수"],
    }

    # 카테고리별 기본 제약 규칙 템플릿
    CONSTRAINT_TEMPLATES = {
        FailureCategory.ITEM_DUPLICATE: (
            "이미 획득한 아이템({items})을 다시 획득하는 장면 금지. '사용' 또는 '소지 확인'으로 대체하세요."
        ),
        FailureCategory.ITEM_MISSING: (
            "획득하지 않은 아이템은 사용 불가. 사용 전 반드시 '획득' 장면이 선행되어야 합니다."
        ),
        FailureCategory.STATE_DISCONTINUITY: ("상태(부상/경지)는 연속적으로 변화해야 함. 직전 화 상태: {prev_state}"),
        FailureCategory.TIMELINE_ERROR: ("수여물/직위는 수여 시점 이후에만 효력 발생. 타임라인을 재확인하세요."),
        FailureCategory.SCOPE_OVERFLOW: (
            "이번 화 분량은 Blueprint 범위 내로 제한. 씬 개수: {scene_count}개, 예상 분량: {expected_length}자"
        ),
        FailureCategory.BLUEPRINT_MISMATCH: ("Blueprint의 모든 핵심 씬을 반영해야 함. 누락된 씬: {missing_scenes}"),
        FailureCategory.RELATIONSHIP_JUMP: (
            "관계 변화는 단계적으로. 한 화에 2단계 이상 변화 금지. 예: 무시→경계→중립 (O), 무시→충성 (X)"
        ),
        FailureCategory.CHARACTER_OOC: ("캐릭터 성격 일관성 유지. {character}의 특성: {traits}"),
        FailureCategory.VILLAIN_STUPIDITY: (
            "악역의 지능 보호. 연속 3회 이상 과소평가 금지. 경계/의심 묘사를 포함하세요."
        ),
        FailureCategory.FREE_POWERUP: ("성장에는 반드시 대가 필요. 예: 고통스러운 수련, 귀인의 도움(빚), 기회비용"),
        FailureCategory.DEUS_EX_MACHINA: ("갑작스러운 해결책 금지. 최소 2화 전에 복선이 있어야 합니다."),
        FailureCategory.PACING_ISSUE: ("호흡 조절 필요. 대화:서술 비율 {ratio} 권장. 긴장/이완 교차를 유지하세요."),
    }
    ...
    def __init__(self, max_records: int = 500):
        ...
        self.recent_failures: dict[int, list[FailureRecord]] = {2: [], 3: [], 4: []}  # 스테이지별 최근 실패 (최대 10개)
    ...
    def record_failure(
        self, stage: int, episode: int, reason: str, arc: int = 0, details: dict[str, Any] = None
    ) -> FailureRecord:
        """실패 기록 추가"""
        category = self.classify_failure(reason)

        record = FailureRecord(
            category=category, stage=stage, episode=episode, arc=arc, reason=reason, details=details or {}
        )
    ...
    def get_top_failures(self, stage: int = None, n: int = 5) -> list[tuple[FailureCategory, int]]:
        """가장 빈번한 실패 카테고리 반환"""
        if stage:
    ...
    def get_learned_constraints(self, stage: int, threshold: int = 2) -> list[LearnedConstraint]:
        """
        학습된 제약 조건 생성
        ...
        for category, count in self.stage_counts[stage].items():
            if count >= threshold and category != FailureCategory.UNKNOWN:
                # 해당 카테고리의 최근 예시 수집
                examples = [r.reason[:100] for r in self.recent_failures[stage] if r.category == category][:3]
                ...
                constraints.append(
                    LearnedConstraint(
                        rule=rule, category=category, frequency=count, examples=examples, priority=priority
                    )
                )
    ...
    def generate_constraint_prompt(self, stage: int, threshold: int = 2) -> str:
    ...
    def get_failure_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "by_stage": {
                stage: {cat.value: count for cat, count in counts.items() if count > 0}
                for stage, counts in self.stage_counts.items()
            },
            "top_5": [{"category": cat.value, "count": count} for cat, count in self.get_top_failures(n=5)],
        }
