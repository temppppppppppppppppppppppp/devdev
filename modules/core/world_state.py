...
class WorldStateManager:
    ...
    def _load_or_init(self) -> dict:
        ...
        try:
            ...
            if loaded and isinstance(loaded, dict) and loaded.get('version'):
                _logger.info("[V68] WorldState: DB에서 로드 완료 (ep %d)",
                             loaded.get('last_updated_ep', 0))
                return loaded
    ...
    def update_from_state_changes(self, ep_num: int, state_changes: dict):
        ...
        try:
            ...
            # 5. 엔티티 파괴 (조직/장소)
            _existing_destroyed_names = {d.get('name') for d in self._state['destroyed'] if isinstance(d, dict)}  # [V70] 중복 방지
            ...
            for plot in (state_changes.get('resolved_plots') or []):
                ...
                if plot_desc:
                    ...
                    self._state['active_plots'] = [
                        p for p in self._state['active_plots']
                        if p.get('plot', '') != plot_desc
                    ]
            ...
            for comp in (state_changes.get('companion_changes') or []):
                ...
                if npc not in self._state['dead_npcs']:
                    if npc not in self._state['alive_npcs']:
                        self._state['alive_npcs'][npc] = {}
                    self._state['alive_npcs'][npc]['companion'] = (action in ('join', 'joined', '합류'))  # [V70] LLM 출력값 호환
    ...
    def update_protagonist_state(self, ep_num: int,
                                  name: str = None,
                                  location: str = None,
                                  assets: str = None,
                                  injuries: str = None):
    ...
    def get_summary(self, max_chars: int = 5000) -> str:
        ...
        try:
            ...
            active_items = {k: v for k, v in items.items()
                           if isinstance(v, dict) and v.get('status', '보유') == '보유'}
            if active_items:
            ...
            if plots:
                plot_lines = [f"- {p.get('plot', '?')} (제{p.get('since_ep', '?')}화~)"
                              for p in plots[-10:]]
    ...
    def register_alive_npc(self, name: str, role: str = "", relation: str = "",
                            location: str = ""):
        """NPC를 생존 목록에 수동 등록"""
