...
class LoreManager:
    ...
    def __init__(self, context) -> None:
        ...
        self._all_lore_cache: list[dict] | None = None
    ...
    def get_lore_by_db_search(self, keyword: str) -> list[dict]:
        ...
        try:
            ...
            """
            pattern = f"%{keyword}%"
            cursor = self.db.cursor.execute(query, (pattern, pattern, self.MAX_LORE_RESULTS))
            results = []
            for row in cursor.fetchall():
                results.append({"category": row[0], "item": row[1], "description": row[2]})
            return results
        except Exception:
            # 테이블이 없거나 쿼리 실패 시 폴백
    ...
    def update_v20_assets(self, new_lore_data) -> None:
        ...
        current_db_lore = {l["item"].replace(" ", "").lower(): l for l in self._get_all_lore_cached()}
        lore_batch = []
    ...
    def sync_equipment_to_encyclopedia(self, old_equipment: list, new_equipment: list, ep_num: int) -> dict:
        ...
        existing_items = {lore["item"].strip().lower() for lore in self.db.get_lore_list_by_category("item") or []}
