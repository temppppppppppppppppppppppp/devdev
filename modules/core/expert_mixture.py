...
class ExpertMixture:
    ...
    SCENE_KEYWORDS = {
        ...
        SceneType.COMEDY: ["유머", "코미디", "사이다", "통쾌", "웃음", "해학", "comedy", "humor", "satisfaction"],
    }
...
5. **가족/친구**: 부와 인간관계""",
    ...
    def get_scene_prompt(self, scene_type: SceneType, context: dict[str, Any] = None) -> str:
        """
        ...
        if not base_prompt:
            ...
            base_prompt = self.EXPERT_PROMPTS["wuxia"].get(
                scene_type, "[씬 작성 가이드] 장면의 목적을 명확히 하고, 감각적 묘사를 포함하세요."
            )
    ...
    def generate_writer_injection(self, blueprint: dict[str, Any], max_prompts: int = 3) -> str:
        """
