...
class DynamicPromptWeighter:
    ...
    FAILURE_KEYWORD_MAP = {
        PromptCategory.CONTINUITY: [
            "연속성", "continuity", "이전 화", "직전", "모순", "불일치",
            "타임라인", "timeline", "시간 역행"
        ],
        PromptCategory.ITEM_MANAGEMENT: [
            "아이템", "item", "획득", "중복", "소지", "무기", "장비",
            "inventory", "duplicate"
        ],
        PromptCategory.RELATIONSHIP: [
            "관계", "relationship", "급변", "역행", "충성", "적대",
            "NPC", "반응"
        ],
        PromptCategory.PACING: [
            "호흡", "pacing", "템포", "서사 폭주", "서사 정체",
            "범위 초과", "scope"
        ],
        PromptCategory.CHARACTER: [
            "캐릭터", "character", "일관성", "OOC", "성격", "말투",
            "voice"
        ],
        ...
        PromptCategory.DIALOGUE: [
            "대화", "dialogue", "대사", "말", "conversation"
        ],
        PromptCategory.DESCRIPTION: [
            "묘사", "description", "감각", "시각", "청각", "sensory"
        ],
        PromptCategory.BLUEPRINT_COMPLIANCE: [
            "blueprint", "블루프린트", "설계", "준수", "compliance"
        ],
        PromptCategory.VILLAIN_INTELLIGENCE: [
            "악역", "villain", "지능", "멍청", "과소평가", "경계"
        ]
    }
    ...
    def get_weighted_directives(
        ...
        for cat in relevant_categories:
            ...
            if template:
                directives.append(WeightedDirective(
                    category=cat,
                    weight=weight,
                    directive=template,
                    urgency=urgency
