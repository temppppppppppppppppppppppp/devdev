...
10. **전문가 반응 캘리브레이션**: 음악인의 반응은 명성·경력에 비례해야 한다. 히트 프로듀서급이 앨범 차트인에 흥분하면 안 된다(일상). 그래미 노미네이션이나 표절 소송에는 동요해야 한다(실제 이변). 전문가는 자기 수준의 일상적 성과에 담담하고, 급을 넘어서는 사건에만 반응한다.
    ...
    def get_impossible_actions(self, current_state: dict[str, Any]) -> list[dict[str, str]]:
        ...
        if fame_class and fame_class in self._fame_action_limits:
            for pattern in self._fame_action_limits[fame_class]:
                actions.append({"pattern": pattern, "reason": f"명성 '{fame_class}'에서 불가", "severity": "HIGH"})
