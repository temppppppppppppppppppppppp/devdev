...
class SportsGuard(BaseGuard):
    ...
    def __init__(self):
        ...
        # 스포츠 업계 전문 용어
        self.SPORTS_TERMS = [
            "드리블", "패스", "슛", "태클", "세트피스", "코너킥",
            "쿼터", "하프타임", "연장전", "승부차기", "PK",
            "심판", "파울", "옐로카드", "레드카드", "VAR",
            "트레이닝", "피지컬", "컨디셔닝", "웨이트", "인터벌",
            "리그", "토너먼트", "플레이오프", "챔피언십", "월드컵",
            "에이전트", "스카우트", "드래프트", "이적료", "연봉",
            "스폰서", "브랜드", "광고", "미디어데이", "기자회견",
            "MVP", "득점왕", "어시스트", "클린시트", "트리플더블",
            "유소년", "아카데미", "2군", "임대", "FA",
            "감독", "코치", "트레이너", "피지오", "닥터",
        ]
        ...
        # [V57] 선수 등급
        self._athlete_hierarchy = [
            '동호회', '아마추어', '세미프로', '프로신인', '프로정규', '올스타', '국가대표', '전설',
        ]

        # [V57] 선수 등급별 행동 불가 패턴
        self._athlete_action_limits = {
...
"""
    ...
    def get_impossible_actions(self, current_state: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        if tier_class and tier_class in self._athlete_action_limits:
            for pattern in self._athlete_action_limits[tier_class]:
                actions.append({
                    'pattern': pattern,
                    'reason': f"선수 등급 '{tier_class}'에서 불가",
                    'severity': 'HIGH'
                })
    ...
    def get_justification_patterns(self) -> List[str]:
        ...
        return [
            ...
            r'체계적.*훈련', r'특별.*트레이닝', r'코치.*지도',
            r'실전.*경험', r'경기.*경험', r'대회.*출전',
            r'멘토.*가르침', r'선배.*조언', r'감독.*신뢰',
            r'천부적.*재능', r'타고난.*신체', r'운동.*신경',
            r'피나는.*노력', r'새벽.*훈련', r'독자.*연습',
            r'부상.*재활', r'재활.*성공', r'컴백.*준비',
            r'미디어.*노출', r'팬.*지지', r'스폰서.*제안',
            r'스카우트.*발탁', r'공개.*테스트', r'트라이아웃',
        ]
    ...
    def get_resolution_patterns(self) -> List[str]:
        """[V57] 스포츠물 - 갈등 해소 패턴"""
        return [
            r'경기.*증명', r'기록.*갱신', r'MVP.*수상',
            r'팀.*단합', r'동료.*화해', r'감독.*인정',
            r'재계약', r'이적.*성공', r'복귀전.*활약',
            r'우승', r'승격', r'챔피언',
            r'재활.*성공', r'컴백.*성공',
            r'팬.*지지', r'여론.*반전',
            r'결백.*증명', r'징계.*해제',
