...
class MedicalGuard(BaseGuard):
    ...
    def __init__(self):
        ...
        self.MEDICAL_TERMS = [
            "바이탈", "혈압", "맥박", "산소포화도", "체온",
            "수술", "절개", "봉합", "지혈", "마취", "전신마취", "국소마취",
            "진단", "소견", "판독", "검사", "혈액검사", "조직검사",
            "CT", "MRI", "X-ray", "초음파", "PET",
            "처방", "투약", "수액", "항생제", "진통제",
            "회진", "컨퍼런스", "차트", "오더", "프로토콜",
            "레지던트", "어텐딩", "인턴", "수련의", "전공의",
            "응급실", "중환자실", "수술실", "병동", "외래",
            "트리아지", "코드블루", "심폐소생술", "제세동기",
            "전과", "전원", "퇴원", "입원", "외래진료",
            "임상시험", "IRB", "인폼드컨센트", "예후",
        ]
        ...
        # [V57] 의사 직급
        self._doctor_hierarchy = [
            '의대생', '인턴', '레지던트', '전문의', '펠로우', '부교수', '교수', '석좌교수',
        ]

        # [V57] 직급별 행동 불가 패턴
        self._doctor_action_limits = {
...
"""
    ...
    def get_justification_patterns(self) -> List[str]:
        ...
        return [
            ...
            r'수술.*집도', r'임상.*경험', r'수련.*과정',
            r'학술.*연구', r'논문.*발표', r'학회.*참석',
            r'해외.*연수', r'유학.*경험', r'명의.*지도',
            r'천부적.*손재주', r'타고난.*감각', r'직관',
            r'밤새.*공부', r'독학.*연구', r'케이스.*분석',
            r'응급.*대응', r'실전.*경험', r'위기.*대처',
            r'멘토.*가르침', r'교수.*지도', r'선배.*조언',
        ]
    ...
    def get_hostile_action_types(self) -> List[str]:
        """[V57] 의학물 - 적대적 행동 유형"""
        return [
            '의료사고 모함', '논문 표절 의혹', '환자 빼앗기',
            '과장 자리 경쟁', '수련의 갈굼', '부당 지시',
            '의료 소송', '내부 고발', '파벌 다툼',
            '제약사 로비', '보험 사기', '은폐 시도',
            '인사 불이익', '연구비 횡령 의혹',
        ]
    ...
    def get_resolution_patterns(self) -> List[str]:
        """[V57] 의학물 - 갈등 해소 패턴"""
        return [
            r'수술.*성공', r'환자.*회복', r'진단.*정확',
            r'논문.*게재', r'학회.*발표', r'연구.*성과',
            r'결백.*증명', r'소송.*승소', r'조사.*무혐의',
            r'동료.*인정', r'교수.*신뢰', r'환자.*감사',
            r'승진', r'임용', r'과장.*취임',
