...
class CookingGuard(BaseGuard):
    ...
    def __init__(self) -> None:
        ...
        # 요리 업계 전문 용어 (정확하게 사용해야 함)
        self.COOKING_TERMS = [
            "마이야르", "캐러멜라이즈", "데글라제", "플랑베", "수비드",
            "블랑쉬", "브레이즈", "소테", "포칭", "콩피",
            "플레이팅", "미장플라스", "가니시", "아뮤즈부쉬",
            "식감", "풍미", "감칠맛", "우마미", "테루아",
            "코스요리", "아뮤즈", "앙트레", "디저트", "프리픽스",
            "미슐랭", "빕구르망", "고메", "파인다이닝", "가스트로펍",
            "원가율", "식재료 수급", "레시피 개발", "메뉴 엔지니어링",
            "주방장", "수셰프", "파티시에", "소믈리에", "바리스타",
            "식품위생", "HACCP", "콜드체인", "숙성", "발효",
        ]
        ...
        # [V57] 셰프 등급
        self._chef_hierarchy = [
            '수습생', '준요리사', '요리사', '수석요리사', '셰프', '명셰프', '거장', '전설의요리인',
        ]

        # [V57] 식당 등급
        self._restaurant_hierarchy = [
            '포장마차', '동네식당', '맛집', '파인다이닝', '미슐랭1스타', '미슐랭2스타', '미슐랭3스타',
        ]

        # [V57] 셰프 등급별 행동 불가 패턴
        self._chef_action_limits = {
...
"""
            ...
            for rank_keyword, patterns in self._chef_action_limits.items():
                if rank_keyword in chef_rank:
                    for pattern in patterns:
                        actions.append({
                            'pattern': pattern,
                            'reason': f"셰프 등급 '{rank_keyword}'에서 불가",
                            'severity': 'HIGH'
                        })
    ...
    def get_justification_patterns(self) -> List[str]:
        ...
        return [
            ...
            # 정당한 성장
            r'연습.*결과', r'멘토.*지도', r'스승.*가르침',
            r'대회.*입상', r'수련.*성과',

            # 식재료 확보
            r'산지.*직거래', r'계약.*재배', r'특수.*루트',
            r'직접.*재배', r'수입.*경로',

            # 재능/노력
            r'천부적.*미각', r'절대.*미각', r'타고난',
            r'밤새.*연구', r'수백.*번.*실험',

            # 합법적 기회
            r'공모전', r'요리.*대회', r'SNS.*입소문',
            r'방송.*출연', r'맛집.*소개',
        ]
    ...
    def get_hostile_action_types(self) -> List[str]:
        """[V57] 요리물 - 적대적 행동 유형"""
        return [
            '레시피 도용', '식재료 가로채기', '악의적 평가',
            '위생 신고', '영업 방해', '인력 빼가기',
            '라이벌 도발', '대회 부정행위', '허위 리뷰',
            '식중독 모함', '가격 덤핑', '공급처 차단',
        ]
    ...
    def get_resolution_patterns(self) -> List[str]:
        ...
        return [
            # 요리로 증명
            r'요리.*실력.*증명', r'맛으로.*승부', r'대회.*우승',
            r'고객.*인정', r'평론가.*극찬',

            # 합의/화해
            r'합의', r'화해', r'협상.*타결', r'협력',

            # 경영 승리
            r'매출.*역전', r'미슐랭.*획득', r'맛집.*선정',
            r'예약.*대기', r'줄.*서는.*맛집',

            # 독립/성장
            r'독립.*오픈', r'자기.*가게', r'독자.*노선',
            r'새로운.*메뉴', r'혁신.*요리',
        ]
