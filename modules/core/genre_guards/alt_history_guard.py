...
class AltHistoryGuard(BaseGuard):
    ...
    def __init__(self):
        ...
        self.FORBIDDEN_TERMS = [
            # 전자기기/IT
            "스마트폰", "컴퓨터", "인터넷", "노트북", "태블릿", "모니터",
            "이메일", "SNS", "앱", "어플", "블로그", "유튜브", "네이버",
            "와이파이", "블루투스", "USB", "프린터", "AI", "인공지능",
            # 근현대 교통
            "자동차", "버스", "지하철", "비행기", "오토바이", "택시", "엘리베이터",
            # 근현대 시설
            "카페", "편의점", "백화점", "마트", "병원", "아파트",
            "영화관", "놀이공원", "체육관",
            # 플라스틱/화학제품
            "플라스틱", "비닐", "나일론", "폴리에스터", "스티로폼",
            # 판타지/게임 용어
            "마나", "스킬", "던전", "게이트", "각성", "레벨업",
            "경험치", "상태창", "시스템 창", "퀘스트", "인벤토리",
            # 무협 용어
            "내공", "진기", "검기", "검강", "초식", "무림", "강호",
        ]

        # 조선 시대 전문 용어 (정확하게 사용해야 함)
        self.JOSEON_TERMS = [
            # 관직 체계
            "영의정", "좌의정", "우의정", "판서", "참판", "참의",
            "이조", "호조", "예조", "병조", "형조", "공조",
            "승정원", "의금부", "사헌부", "사간원", "홍문관",
            "암행어사", "목사", "부사", "군수", "현감",
            # 당파
            ...
            "양반", "중인", "상민", "천민", "사대부", "서얼",
            # 과거 제도
            "과거", "문과", "무과", "잡과", "생원", "진사", "급제", "장원",
            # 예법/문화
            "삼강오륜", "충효", "예법", "존비", "항렬",
            "한양", "성균관", "사직", "종묘", "경복궁", "창덕궁",
        ]
        ...
        # [V57] 품계 위계 (정1품 ~ 종9품)
        self._court_rank_hierarchy = [
            '종9품', '정9품', '종8품', '정8품', '종7품', '정7품',
            '종6품', '정6품', '종5품', '정5품', '종4품', '정4품',
            '종3품', '정3품', '종2품', '정2품', '종1품', '정1품',
        ]

        # [V57] 신분 위계
        self._social_hierarchy = ['천민', '상민', '중인', '양반', '왕족']

        # [V57] 신분별 불가 행동 패턴
        self._class_action_limits = {
...
"""
    ...
    def get_justification_patterns(self) -> List[str]:
        ...
        return [
            # 미래 정보 활용 (회귀물)
            r'미래.*기억', r'전생.*경험', r'과거.*지식',

            # 정당한 승진/임명
            r'과거.*급제', r'공적.*인정', r'왕명.*임명',
            r'천거.*받', r'상소.*공로',

            # 신분 초월 근거
            r'왕.*특명', r'공신.*책봉', r'서얼.*허통',
            r'면천.*허락', r'양인.*등록',

            # 기술 도입 근거
            r'서양.*문물', r'청.*통해', r'역관.*소개',
            r'실학.*연구', r'오랜.*실험',

            # 정치적 명분
            r'상소.*근거', r'경연.*논의', r'선례.*따라',
            r'대의.*명분', r'유교.*가르침',
        ]
    ...
    def get_resolution_patterns(self) -> List[str]:
        ...
        return [
            # 정치적 승리
            r'왕.*편', r'탄핵.*성공', r'복권', r'사면',
            r'관직.*복귀', r'공신.*책봉',

            # 명분 승리
            r'상소.*관철', r'경연.*설득', r'여론.*전환',
            r'민심.*얻', r'유생.*지지',

            # 실력 증명
            r'과거.*장원', r'외교.*성공', r'전쟁.*승리',
            r'기술.*성공', r'민생.*안정',

            # 타협/화해
            r'화해', r'연립', r'타협', r'혼인.*동맹',
            r'당론.*조율', r'상호.*양보',
        ]
