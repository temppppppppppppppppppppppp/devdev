...
class ActorGuard(BaseGuard):
    ...
    def __init__(self) -> None:
        ...
        self.ENTERTAINMENT_TERMS = cfg.get(
            "entertainment_terms",
            [
                "오디션",
                "캐스팅",
        ...
        self._fame_action_limits = cfg.get(
            "fame_action_limits",
            {
...
10. **전문가 반응 캘리브레이션**: 연예계 인물의 반응은 경력·지위에 비례해야 한다. 톱스타급이 지상파 캐스팅에 흥분하면 안 된다(일상). 할리우드 오퍼나 전례 없는 스캔들에는 동요해야 한다(실제 이변). 전문가는 자기 레벨의 일상사에 담담하고, 급을 넘어서는 사건에만 반응한다.
    ...
    def get_impossible_actions(self, current_state: dict[str, Any]) -> list[dict[str, str]]:
        ...
        if fame_class and fame_class in self._fame_action_limits:
            for pattern in self._fame_action_limits[fame_class]:
                actions.append({"pattern": pattern, "reason": f"인지도 '{fame_class}'에서 불가", "severity": "HIGH"})
