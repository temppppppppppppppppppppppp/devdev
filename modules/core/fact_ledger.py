...
class FactLedger:
    ...
    def update_from_state_changes(self, ep_num: int, state_changes: dict):
        ...
        for death in (state_changes.get('npc_deaths') or []):  # [V70] None 방어
            ...
            name = death.get('name', '')
            if name:
                self._upsert_character(name, ep_num,
                    status='dead',
                    note=f"사망 (원인: {death.get('cause', '불명')})")
        ...
        for rel in (state_changes.get('relationship_changes') or []):  # [V70] None 방어
            ...
            npc = rel.get('npc', '')
            if npc:
                self._upsert_character(npc, ep_num,
        ...
        for skill in (state_changes.get('skill_acquisitions') or []):  # [V70] None 방어
            ...
            name = skill.get('name', '')
            if name:
                self._upsert_item(name, ep_num,
                    owner='주인공', status='습득',
                    note=f"습득 (출처: {skill.get('source', '불명')})")
        ...
        for item in (state_changes.get('major_items') or []):  # [V70] None 방어
            if isinstance(item, dict):
                ...
                continue
            if iname:
                self._upsert_item(iname, ep_num,
                    owner='주인공', status=action,
                    note=f"{action}")
        ...
        for dest in (state_changes.get('entity_destructions') or []):  # [V70] None 방어
            ...
            if name:
                if dtype == 'organization':
                    self._upsert_org(name, ep_num, status='destroyed',
                        note=f"파괴 (원인: {dest.get('cause', '불명')})")
                else:
                    self._upsert_location(name, ep_num, status='destroyed',
                        note=f"파괴 (원인: {dest.get('cause', '불명')})")
        ...
        for injury in (state_changes.get('npc_injuries') or []):  # [V70] None 방어
            ...
            if name:
                ...
                self._upsert_character(name, ep_num,
                    note=f"부상: {injury_type}" if injury_type else "부상")
        ...
        for rel in (state_changes.get('npc_npc_relationships') or []):  # [V70] None 방어
            ...
            if npc1 and npc2 and relation:
                self._upsert_character(npc1, ep_num,
                    note=f"{npc2}와 관계: {relation}")
                self._upsert_character(npc2, ep_num,
                    note=f"{npc1}와 관계: {relation}")
    ...
    def update_from_bible_delta(self, ep_num: int, bible_delta: dict):
        ...
        for npc_name in (bible_delta.get('new_npcs') or []):  # [V70] None 방어
            if isinstance(npc_name, str) and npc_name:
                self._upsert_character(npc_name, ep_num,
                    note="신규 등장")
        ...
        for item_name in (bible_delta.get('new_items') or []):  # [V70] None 방어
            if isinstance(item_name, str) and item_name:
                self._upsert_item(item_name, ep_num,
                    owner='주인공', status='획득',
                    note="신규 획득")
    ...
    def _upsert_character(self, name: str, ep_num: int,
                          status: str = None, role: str = None,
                          relationship: str = None, note: str = ""):
        """캐릭터 정보 생성 또는 갱신."""
        chars = self._ledger['characters']
        if name not in chars:
            chars[name] = {
                'status': 'alive', 'role': '', 'relationship': '',
                'established_ep': ep_num, 'last_ep': ep_num, 'history': []
            }
        entry = chars[name]
        if status:
            entry['status'] = status
    ...
    def _upsert_item(self, name: str, ep_num: int,
                     owner: str = None, status: str = None, note: str = ""):
        """아이템/무공 정보 생성 또는 갱신."""
        items = self._ledger['items']
        if name not in items:
            items[name] = {
                'owner': owner or '', 'status': status or '보유',
                'established_ep': ep_num, 'last_ep': ep_num, 'history': []
            }
        entry = items[name]
        if owner:
    ...
    def _upsert_location(self, name: str, ep_num: int,
                         status: str = None, current_owner: str = None, note: str = ""):
        """장소 정보 생성 또는 갱신."""
        locs = self._ledger['locations']
        if name not in locs:
            locs[name] = {
                'status': status or '정상', 'current_owner': current_owner or '',
                'last_ep': ep_num, 'history': []
            }
        entry = locs[name]
        if status:
            entry['status'] = status
    ...
    def _upsert_org(self, name: str, ep_num: int,
                    status: str = None, leader: str = None, note: str = ""):
        """조직 정보 생성 또는 갱신."""
        orgs = self._ledger['organizations']
        if name not in orgs:
            orgs[name] = {
                'status': status or '활동중', 'leader': leader or '',
                'last_ep': ep_num, 'history': []
            }
        entry = orgs[name]
        if status:
    ...
    def to_summary(self, max_chars: int = None) -> str:
        ...
        if chars:
            alive = {k: v for k, v in chars.items() if isinstance(v, dict) and v.get('status') == 'alive'}  # [V70] non-dict 방어
        ...
        if items:
            active_items = {k: v for k, v in items.items() if isinstance(v, dict) and v.get('status') not in ('분실', '파괴', '소모')}  # [V70] dict 방어
            lost_items = {k: v for k, v in items.items() if isinstance(v, dict) and v.get('status') in ('분실', '파괴', '소모')}  # [V70] dict 방어
            ...
            if active_items:
                ...
                for name, info in list(active_items.items())[:20]:
                    ...
                    parts.append(f"  - {name} ({info.get('status', '보유')}{owner_str}, ep{info.get('established_ep', '?')})")
        ...
        if locations:
            destroyed_locs = {k: v for k, v in locations.items() if isinstance(v, dict) and v.get('status') == 'destroyed'}  # [V70] dict 방어
            active_locs = {k: v for k, v in locations.items() if isinstance(v, dict) and v.get('status') != 'destroyed'}  # [V70] dict 방어
        ...
        if orgs:
            destroyed_orgs = {k: v for k, v in orgs.items() if isinstance(v, dict) and v.get('status') == 'destroyed'}  # [V70] dict 방어
            active_orgs = {k: v for k, v in orgs.items() if isinstance(v, dict) and v.get('status') != 'destroyed'}  # [V70] dict 방어
