...
class FeedbackSystem:
    ...
    def build_structured_feedback(self, decision: str, reason: str, violations: list = None,
...
""".strip()
    ...
    def build_strong_kind_feedback_legacy(self, violations: list, attempt: int, protagonist_name: str = "주인공") -> str:
...
→ 이미 가진 아이템 다시 획득 금지
    ...
    def generate_structured_blueprint_feedback(self, director_result: dict, blueprint: dict = None, retry_count: int = 0) -> str:
        ...
        if setting_score < 15:
            issues.append({
                'priority': 'CRITICAL', 'area': '설정 일관성', 'score': setting_score,
                'action': '미습득 무공/미획득 아이템 사용 장면 삭제. HUD 확인 필수.'
            })
        elif setting_score < 18:
            issues.append({
                'priority': 'HIGH', 'area': '설정 일관성', 'score': setting_score,
        ...
        if scene_score < 10:
            issues.append({
                'priority': 'CRITICAL', 'area': '장면 구성', 'score': scene_score,
                'action': f'장면 수 부족. 최소 5개 이상 설계 필요.'
            })
        elif scene_score < 15:
            issues.append({
                'priority': 'HIGH', 'area': '장면 구성', 'score': scene_score,
        ...
        if flow_score < 10:
            issues.append({
                'priority': 'CRITICAL', 'area': '서사 흐름', 'score': flow_score,
                'action': '서사 폭주/정체 감지. 사건 분배 재조정 필요.'
            })
        elif flow_score < 15:
            issues.append({
                'priority': 'HIGH', 'area': '서사 흐름', 'score': flow_score,
        ...
        if length_score < 10:
            issues.append({
                'priority': 'CRITICAL', 'area': '분량', 'score': length_score,
    ...
    def generate_reverse_feedback_stage4_to_3(self, writer_reject_reason: str, pre_checklist_result: dict = None) -> str:
    ...
    def get_adaptive_feedback_intensity(self, retry_count: int, stage: int = 4) -> dict:
        ...
        if stage == 2:
            if retry_count == 0:
                return {
                    'pass_threshold': 70, 'feedback_level': 'detailed',
                    'strictness': 'high',
                    'guidance': '완벽한 Arc 설계를 목표로 합니다. 모든 제약 조건을 준수하세요.'
                }
            elif retry_count == 1:
                return {
                    'pass_threshold': 65, 'feedback_level': 'focused',
                    'strictness': 'medium',
                    'guidance': 'CRITICAL 이슈만 해결하세요. 부가적 품질은 차후 개선.'
                }
            else:
                return {
                    'pass_threshold': 55, 'feedback_level': 'minimal',
                    'strictness': 'low',
            ...
            if retry_count == 0:
                ...
                return {
                    'pass_threshold': 60, 'feedback_level': 'focused',
                    'strictness': 'medium',
                    'guidance': '핵심 씬 5개 확보, 설정 일관성 유지에 집중.'
                }
            else:
                return {
                    'pass_threshold': 50, 'feedback_level': 'minimal',
                    'strictness': 'low',
