...
### 📚 [V67] 이전 원고 전문 — 진실의 원천 (최대 30화)
아래는 이전에 확정·출판된 원고 전문입니다. 이 내용이 "실제로 일어난 일"입니다.
현재 후보 원고가 아래 사실과 모순되면 반드시 REJECT하세요.
...
### 🚨🚨🚨 [V67] 최우선 — 명시적 모순 검사 (반드시 먼저 수행)
후보 평가 전에, 각 후보를 이전 원고 전문과 대조하여 아래 항목을 검사하세요:

1. **고유명사 일관성**: 인물명, 조직명, 장소명, 회사명이 이전 원고와 동일한가?
   예: 이전 화에서 "한미증권"이었는데 현재 "대민증권"으로 바뀌면 → REJECT
2. **수치 연속성**: 금액, 가격, 환율, 수량 등 숫자가 이전 원고와 합리적으로 연결되는가?
   예: 이전 화에서 "73억"이었는데 설명 없이 "15억"으로 줄면 → REJECT
3. **사망/파괴 존중**: 이전 원고에서 죽은 NPC, 파괴된 장소/조직이 다시 등장하지 않는가?
4. **아이템/능력 일관성**: 이전에 잃어버린 아이템 재사용, 미습득 무공 사용이 없는가?
5. **상태 연속성**: 부상, 감정 상태, 관계가 이전 원고와 일관되는가?
6. **타임라인 정합성**: 시간 흐름이 자연스러운가? 순간이동이 없는가?
7. **수학적 정확성** (투자물 CRITICAL 항목):
   - **레버리지 수익률 공식**: 계좌 수익률(%) = 기초자산 변동폭(%) × 레버리지 배수
     예) 진입가 60.15달러 → 매도가 64.20달러 = 기초자산 6.73% 상승. 3배 레버리지 시 → 계좌 수익률 20.2% (40.6%로 쓰면 CRITICAL 오류)
   - 진입가·매도가·레버리지·수익률이 동시에 명시된 경우 공식으로 검증하라.
   - 불일치 시: CRITICAL 모순 → [I-10] 규칙 적용 (연속성 점수 최대 15/40)
   - **자본금 정합**: 수익 후 지출(보증금·세금 등)이 있으면 잔액 = 원금+수익-지출 확인
8. **[V69.1] 중복 서술 금지**: 직전 화의 마지막 장면(대화·행동·이동)을 현재 화 도입부에서 다시 서술하고 있지 않은가? 같은 이벤트가 두 화에 걸쳐 반복되면 → 감점

위 8개 항목 중 1개라도 CRITICAL 모순이 발견되면 해당 후보는 즉시 0점 처리하세요.

⚠️ 중요: 위 모순 검사 시 아래 작품 설정을 반드시 고려하세요:
- **회귀자**: 미래의 정보를 알고 있거나 역사를 의도적으로 변경하는 것은 모순이 아닙니다. 회귀자가 "아직 일어나지 않은 사건"을 예측하거나 대비하는 것은 설정상 정상입니다.
- **빙의자**: 원래 인물의 기억과 현재 인격이 다를 수 있습니다. 이것은 모순이 아닙니다.
- **환생자**: 전생의 기억이 단편적으로 나타나는 것은 모순이 아닙니다.
- 단, 현재 타임라인에서 확립된 사실(사망, 파괴, 아이템 분실 등)은 회귀자/빙의자/환생자에 관계없이 반드시 존중해야 합니다.

### 📖 [V67.2] 자유 형식 모순 리뷰
위 7개 체크리스트 외에도, **이전 원고 전문을 읽은 독자의 시각**에서 각 후보를 검토하세요:
- "이전 화에서 일어난 일과 지금 원고가 자연스럽게 이어지는가?"
- "독자가 읽다가 '어? 이게 맞나?' 하고 고개를 갸우뚱할 부분이 없는가?"
- 구체적인 사실 모순이 아니더라도, 톤/분위기/인물 태도의 급변, 동기 없는 행동 전환, 설명 없는 상황 변화 등 **어색한 부분**이 있으면 지적하세요.
- **전문가 반응 비례성**: 전문 직업 캐릭터(PB, 의사, 셰프, 선수, 무인 등)가 자기 분야의 일상적 상황에 과잉 반응하고 있지 않은가? (예: VIP PB가 20억에 놀람→잘못, 3배 레버리지에 놀람→맞음) 위반 시 open_review에 기재.
- [작품 정체성 감리] mandatory_context에 해당 블록이 있으면 `mandatory_scene_engines` 누락, `tracking_slots` 소실, `forbidden_flattenings` 위반을 `work identity drift`로 open_review에 기재.
- 발견한 사항을 아래 JSON의 `open_review` 필드에 자유롭게 서술하세요.

---

### 🎯 [V67] 평가 기준 (가중치)
1. **이전 원고 대비 연속성 + 모순 검사 (40%)**: 위 7개 항목 검사 결과. 이전 원고에서 확립된 사실과 모순이 없는가? 모순이 1건이라도 있으면 해당 후보의 이 항목 점수를 0으로 부여하세요.
2. **Blueprint 장면화 충실도 (20%)**: 설계된 핵심 씬/의무가 실제 장면으로 구현되었는가? 후반부 핵심 씬도 요약 없이 체류하는가?
3. **문장 품질 + 몰입도 (20%)**: 독자가 다음 화를 기다리게 만드는가? 문장이 유려하고 긴장감이 있는가?
   전문 직업 캐릭터가 자기 분야의 루틴에 과잉 반응하면 몰입도 감점 요소.
4. **분량 충족 (10%)**: 5,000자 이상인가? (최소 4,000자)
5. **Python 경고 반영 (10%)**: Python 사전 검증에서 경고가 많은 후보는 감점하세요.

### 🚨 자동 REJECT 조건 (어느 후보든)
- 이전 원고에서 확립된 고유명사/수치/상태와 모순이 있는 경우
- 죽은 NPC가 활동하는 경우
- 미습득 무공/아이템을 사용하는 경우
- 분량 4,000자 미만
- Blueprint 씬 50% 이상 누락
- 원고 내부에서 앞뒤가 맞지 않는 명백한 모순
- 수학적 계산 오류 (금액, 수익률, 수량 등)
...
{{
    ...
    "score": 0-100,
    "score_breakdown": {{
        "continuity_contradiction": 0-40,
        "blueprint_coverage": 0-20,
        "quality_engagement": 0-20,
        "length": 0-10,
        "python_warnings": 0-10
    }},
    "contradiction_check": {{
        "found_contradictions": [
            {{
                "type": "고유명사|수치|사망|아이템|상태|타임라인|수학",
                "prev_fact": "이전 원고에서 확립된 사실",
                "current_violation": "현재 후보에서의 위반",
                "severity": "CRITICAL|MAJOR|MINOR",
                "prev_episode": "이전 원고 회차"
            }}
        ],
        "check_summary": "모순 검사 결과 요약"
    }},
    "open_review": "체크리스트 외 어색함/비약/톤 급변/AI 티/work identity drift를 서술. 없으면 '특이사항 없음'",
    "feedback": {{
        "strengths": ["강점 1", "강점 2"],
        "issues": ["문제점 1 (있을 경우)"],
        "action_items": ["REJECT 시 필수 수정 사항 — 구체적으로 어느 화의 무엇이 모순인지 명시"]
    }},
...
⚠️ 주인공이 회귀자인 경우, 미래 사건을 미리 알거나 역사를 바꾸려는 행동은 "충돌"이 아닙니다.
단, 현재 타임라인에서 이미 발생한 사실(사망, 파괴 등)과 모순되면 충돌입니다.
...
4. **미래 오염 차단 (Future Contamination Guard)**:
   ...
   - 단, 주인공이 회귀자(incarnation_type='회귀자')인 경우, 주인공이 미래를 아는 것은 설정상 정상입니다. 이 경우 '미래 오염'은 NPC가 모를 정보를 NPC가 아는 것만 해당합니다.
...
1. **[BLUEPRINT/MANUSCRIPT 모드 공통]**:
   ...
   - **[페이싱 및 흐름 관리]**:
     ...
     (C) **장면 수 체크 (유연한 기준)**:
         - 기본 목표는 가변 장면 구성이며, 보통 3~6개 장면이 자연스럽다. 장면 수 자체를 절대 권위로 삼지 마라.
         - 2~3개 장면이라도 각 씬의 goal/summary 또는 key_events가 구체적이고 integrated_scenario가 충분히 충실하면 장면 수만으로 REJECT하지 마라.
         - 1개 이하거나, 2~3개 장면이라도 밀도/구체성이 부족하면 REJECT.
...
1. Architect가 설계한 장면 단위가 핵심 obligation 기준으로 모두 실제 장면화되었는가? scene_breakdown이 2~3개여도 각 scene의 goal/summary/key_events가 살아 있으면 장면 수만으로 감점하지 마라.
2. 특정 장면이 생략되거나 후반부로 갈수록 묘사 밀도와 체류 시간이 급격히 떨어지지 않는가?
3. 초반 장면만 상세하고 후반 장면이 급격히 요약·비약되었다면, 분량이나 장면 수와 무관하게 REJECT하라. "몇 씬인가"보다 "무엇이 실제 장면으로 보이는가"를 우선 검수하라.
...
- 📄 최근 회차 실제 본문 (최대 30화): {prev_full_text} 👈 (중복 방지 + 모순 탐지를 위해 반드시 참조! 이전 원고에서 확립된 고유명사/수치/상태와 현재 원고가 일치하는지 대조하라.)
...
Step 2: 장면 구성 평가
- 설계된 장면/의무가 실제 장면으로 충실히 materialize되었는가?
- 후반부 장면의 밀도와 체류 시간이 초반부보다 급격히 무너지지 않는가?
- 장면 수가 적더라도 각 scene의 goal/summary/key_events가 충분히 장면화되었는가?
...
Step 4.5: [V67.2] 자유 형식 모순 리뷰
- 위 체크리스트와 별개로, 독자 시점에서 현재 원고를 통독하라.
- 사실 모순이 아니어도 톤 급변, 태도 급변, 동기 없는 행동 전환은 지적하라.
- 전문가는 루틴보다 진짜 이상 신호에만 반응해야 한다. 위반 시 open_review에 기재.
- [작품 정체성 감리] 해당 블록이 있으면 scene engine/slot/flattening 이탈을 `work identity drift`로 open_review에 기재.
- 발견 사항을 JSON의 `open_review` 필드에 서술하라.
...
2. **장면 구성 (20점)**:
   - 참고: 2~3개 씬 구성도 obligation materialization과 later-scene density가 충분하면 고득점 가능하다.
   - 20점: blueprint 핵심 obligation이 모두 실제 장면으로 구현되고 후반부까지 체류 시간이 유지됨
   - 16점: 핵심 obligation 대부분 구현, 밀도 약간 불균등하나 후반부 요약은 아님
   - 12점: 핵심 obligation은 대체로 구현되지만 한두 지점이 설명 위주로 지나감
   - 8점: 핵심 obligation 일부만 구현되거나 압축 징후가 남음 (재시도 2회+ 시만 PASS 가능)
   - 0점: 1개 씬 이하이거나 핵심 obligation 누락, 또는 후반부 collapse
...
4. **독자 몰입도 (20점)**:
   ...
   - 0점: 다음 화를 읽을 동기 없음
   - **전문가 반응 캘리브레이션**: 전문 직업 캐릭터가 자기 영역의 루틴 사안에 과도하게 놀라거나 동요하면 독자 몰입 파괴 → 감점 요소
...
{{
    ...
    "open_review": "추가 이상(어색함/AI 티/work identity drift). 없으면 '특이사항 없음'",
