...
class ManuscriptValidator:
    ...
    def validate_candidate(
        ...
        # 9. [V63.1] 호칭 일관성 체크
        honorific_result = self._check_honorific_consistency(
            manuscript, prev_manuscript, recent_manuscripts
        )
        ...
        if self._genre_type == 'investment':
            financial_result = self._check_financial_numbers(
                manuscript, recent_manuscripts
            )
        ...
        v631_warnings = (
            ...
            + honorific_result.get("warnings", [])
            + financial_warnings
        )
        v631_focus = (
            meta_result.get("focus_points", [])
            + honorific_result.get("focus_points", [])
            + financial_focus
        )

        if v631_warnings and self._llm_client:
            verified = self._llm_verify_warnings(
                warnings=v631_warnings,
                manuscript_excerpt=manuscript[:3000],
                check_type="combined"
            )
        ...
        for part, injury_pos in injured_parts.items():
            ...
            if '팔' in part or '손' in part:
                use_re = re.compile(
                    ...
                    + re.escape(part) + r'[으로]*\s*(?:들|잡|쥐|휘둘|내려[쳤치])'
                )
            elif '다리' in part:
                use_re = re.compile(
                    re.escape(part) + r'[으로]*\s*(?:걸[었어]|뛰|달[렸려]|차[았올]|밟)'
                )
            else:
                continue
    ...
    def _check_cross_episode_duplication(
        self, manuscript: str, recent_manuscripts: List[Dict]
    ) -> Dict:
        """
        [V62.7] 크로스 에피소드 씬 중복 검사
    ...
    def _check_meta_references(self, manuscript: str) -> Dict:
        ...
        for pattern in self.META_PATTERNS:
            ...
            for m in matches:
                ...
                warnings.append(
                    f"[V63.1] 메타 참조 의심: '{match_text}' (4벽 파괴/에피소드 번호 노출)"
                )
    ...
    # 서술부: ~라고 불렀다 / ~이라 칭했다
    HONORIFIC_NARRATION_PATTERN = re.compile(
        r'([가-힣]{2,6})[이라]고?\s*(?:불렀|칭했|호칭)'
    )
