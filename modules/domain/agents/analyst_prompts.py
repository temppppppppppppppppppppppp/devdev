"""[V65] Analyst 프롬프트 템플릿 외부화."""

# =================================================================
# [V65] 모듈-레벨 상수 5개 (analyst.py에서 이동)
# =================================================================
...
2. 상태 보존: A에서 소모한 핵심 수치나 입은 부상이 B의 도입부에 즉시 반영되었는지 확인하라.
...
2. 상태 그림자(Status Shadow): 이번 사건으로 인해 주인공이 입을 '예상 손실(핵심 수치 소모, 부상 부위, 소모한 영약)'을 수치와 텍스트로 산출하라.
...
- 부상/상태: 물리적 부상은 회복 없이 활동 불가. 회복 장면 필수. 단, 정신적 피로/스트레스는 수면·식사·대화 등 일상 활동 1문장으로 회복 가능하며 opening beat 강제는 아니다.
- 소지품: 이미 있으면 다시 획득 금지.
- 위치: 순간이동 금지, 이동 과정 명시.
...
2. **부상 연속성**: 직전 Arc에서 입은 부상/핵심 수치 소모는 현재 Arc 도입부에 반드시 반영되어야 한다.
...
- 직전 Arc 종료 시 주인공의 부상/핵심 수치 상태는 어떠한가?
...
{{
    ...
    ▶ 시작 상태 (이전 화 종료 상태 계승):
       - 위치: {{이전 화 종료 위치}}
       - {episode_state_label}: {{이전 화 종료 수치}}
       - 부상: {{이전 화 종료 부상}}
       - 소지품: {{이전 화 종료 소지품}}
    ...
    2. {energy_stat_name}: 수치 계산을 명시적으로 작성하라.
       ❌ 수치 오류: 30에서 9를 빼면 21인데 210으로 기록 (산수 오류)
       ✅ 정확한 계산: 30 - 9 = 21 (계산 과정 명시)
...
def get_volume_strategy_prompt() -> str:
    """[V65] design_volume_strategy() 내부 template 반환."""
    return """
        [Role] 전설적인 웹소설 기획자
...
def get_surgery_prompt(*, prev_arc_json: str, curr_arc_json: str, next_arc_json: str, feedback: str) -> str:
    """[V65] analyze_context() ARC_RECONSTRUCTION 내부 surgery_prompt 반환.

    Args:
        prev_arc_json: json.dumps(prev_arc) 결과 또는 "데이터 없음(도입부)"
        curr_arc_json: json.dumps(curr_arc) 결과
        next_arc_json: json.dumps(next_arc) 결과 또는 "데이터 없음(종결부)"
        feedback: 디렉터 피드백 문자열
    """
    return f"""
...
- [이전 아크(Past)]: {prev_arc_json}
- [현재 수술 대상(Present)]: {curr_arc_json}
- [이후 아크 목표(Future)]: {next_arc_json}
...
def get_calibration_prompt(*, calibration_msg: str, current_hud_json: str, arc_tactical: str) -> str:
    """[V65] perform_v35_calibration() 내부 calibration_prompt 반환.

    Args:
        calibration_msg: 보정 메시지 문자열
        current_hud_json: json.dumps(current_hud) 결과
        arc_tactical: 아크 전술 문자열
    """
    return f"""
        [🚨 SYSTEM ADAPTATION: BIBLE CALIBRATION]
        {calibration_msg}

        [현재 상태]: {current_hud_json}
