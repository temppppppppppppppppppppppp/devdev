# synthesis

Role:

- hold the normalized operator handoff between reusable research material and canonical pitch freeze
- store one-page fresh-candidate syntheses assembled from engine packs, normalized work material packs, and bounded research evidence
- give `pitch-selection-checklist.md` one stable input lane before anything is promoted into `canon/`

What this lane is:

- upstream of `canon/`
- downstream of `10_research/`
- operator-facing and selection-facing
- compact by design

What this lane is not:

- not canonical pitch truth
- not a legacy intake dump
- not a quarantine bucket
- not a substitute for `work_guard.yaml`

Normalized route:

- `10_research/20_fewshot_bank` and other reusable research packs
- `10_research/30_work_materials/{work_id}/90_material_pack.json` when a normalized work pack exists
- `20_pitch/synthesis/` one-page synthesis
- `20_pitch/pitch-selection-checklist.md` audit
- `20_pitch/canon/` pitch freeze
- `20_pitch/work-guard-translation-map.md` -> work-specific `work_guard`

Minimum synthesis shape:

- one-line premise
- why now
- protagonist position
- protagonist edge
- stay method or continued-stage reason when relevant
- first proof scene
- first block reward
- contamination risk or must-not-copy note

Operating rule:

- keep the synthesis short enough to act as a selection handoff, not a second canon
- use this lane when assembling a fresh candidate from multiple research engines
- current active operator lane is `male protagonist only`
- female-protagonist ideas can be analyzed as reference or archive material, but do not enter active synthesis promotion
- if the idea passes checklist review, promote the truth into `canon/`
- if the idea fails, keep the notes here or discard them; do not bypass canon by translating directly into a `work_guard`

Starter templates:

- `one_page_synthesis_template.md` = generic fresh-candidate one-page template
- `investment_one_page_synthesis_template.md` = investment-lane practical template
