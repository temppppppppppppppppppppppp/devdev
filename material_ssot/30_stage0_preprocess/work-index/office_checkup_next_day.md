# office_checkup_next_day

Title: 검진 다음 날부터
Family: blockguide
Status: bootstrap manifest

## 1. Stage Snapshot

- research: normalized work-level material pack present
- pitch: mapped through `material_ssot/20_pitch`
- stage0 preprocess: present
- phase0 design: present
- TR: present
- BI: present

## 2. Current Live Chain

- research material pack: `material_ssot/10_research/30_work_materials/office_checkup_next_day/90_material_pack.json`
- preprocess dir: `treatments/preprocess/office_checkup_next_day`
- preprocess manifest: `treatments/preprocess/office_checkup_next_day/source_manifest.json`
- preprocess profile lock: `treatments/preprocess/office_checkup_next_day/profile_lock.json`
- preprocess bundle summary: `treatments/preprocess/office_checkup_next_day/material_bundle_summary.json`
- phase0 snapshot: `treatments/preprocess/office_checkup_next_day/phase0_ready_snapshot.json`
- live phase0 file: `treatments/phase0/office_checkup_next_day_phase0_design.json`
- live TR file: `treatments/07_office_checkup_next_day_tr_block_070_draft.json`
- live BI file: `bible/07_bi_office_checkup_next_day.json`

## 3. Current Notes

- source manifest title: 검진 다음 날부터
- primary profile: `office_power_profile`
- secondary profile: `business_growth_profile`
- research-stage normalized material pack is now available for upstream handoff
- preprocess readiness: immediate Phase 0 handoff is possible with a few naming details still to be fixed during design
- canonical status: current live TR and BI are treated as canonical outputs

## 4. Deferred Links

- research manifest: `material_ssot/10_research/manifests/office_checkup_next_day.md`
- pitch anchor: `material_ssot/20_pitch/canon/office_checkup_next_day.md`
- next stage manifest: `material_ssot/40_phase0_design/work-index/office_checkup_next_day.md`
