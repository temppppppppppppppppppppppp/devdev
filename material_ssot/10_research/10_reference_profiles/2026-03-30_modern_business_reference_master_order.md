...
> 2026-04-03 migration note: canonical sink and manifest moved under `material_ssot/10_research`
...
- 작품별 A/B 트랙을 분리한다
- 결과는 `20_fewshot_bank/cards/` 아래 파일로 모은다
...
| 우선순위 | 작품 | 기능 | NAS 경로 |
| --- | --- | --- | --- |
| 1 | `대한민국 절대 재벌` | 정통 재벌 스케일, 국내 사업 지배력, 거물화 이미지 | `\\172.16.10.120\소설사업부\판무팀_ssot\02_연재\대한민국 절대 재벌(백범(白凡))\1_원고` | <!-- utf8-hygiene: allow-line intentional mixed-script archival NAS path -->
...
공통 소스 규칙:
- source scope는 `nas_only`다.
- 이번 턴은 `\\172.16.10.120\소설사업부\판무팀_ssot\02_연재\대한민국 절대 재벌(백범(白凡))\1_원고`만 사용한다. <!-- utf8-hygiene: allow-line intentional mixed-script archival NAS path -->
- 작품명이 1글자라도 다르면 즉시 FAIL이다.
- ep1/Block 1 접근 실패 시 추정/보간하지 말고 즉시 FAIL이다.
