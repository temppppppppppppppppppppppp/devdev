...
| Pitch | `material_ssot/20_pitch` | `material_ssot/20_pitch/synthesis` for normalized operator syntheses, `material_ssot/20_pitch/canon` for anchors, and `material_ssot/20_pitch/intake/legacy_import` for migrated payloads | none | `전처리_ssot/docs/10_pitches` | none | none | old `전처리_ssot/기획안` payloads were moved into the stage SSOT; QA summaries are quarantined |
...
| Phase 0 design | `material_ssot/40_phase0_design` | preferred root file: `treatments/phase0/{work_id}_phase0_design.json` | `treatments/phase0/{work_id}_phase0_design.json` or preprocess fallback | none | none | none | fallback source is `treatments/preprocess/{work_id}/phase0_ready_snapshot.json`; legacy root `treatments/{work_id}_phase0_design.json` is compatibility-only |
| TR | `material_ssot/50_tr` | `treatments/NN_{work_id}_tr_block_070_draft.json` for numbered live works, otherwise `treatments/{work_id}_tr_block_070_draft.json` | same as canonical | none | none | none | router accepts both numbered and legacy stems, but numbered live works are now preferred for browse visibility |
| BI | `material_ssot/60_bi` | `bible/NN_bi_{work_id}.json` for numbered live works, otherwise `bible/0_bi_{work_id}.json` | same as canonical | none | none | none | router accepts both numbered and legacy stems, but numbered live works are now preferred for browse visibility |
